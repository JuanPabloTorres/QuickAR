...
export default function FunctionalARViewer({
    ...
    const checkARSupport = async () => {
      if (typeof navigator !== "undefined" && navigator.xr) {
        try {
          ...
          if (!supported) {
            onError?.(
              "WebXR AR no está soportado en este dispositivo/navegador"
            );
          }
        } catch (error) {
  ...
  const startARSession = async () => {
    ...
    try {
      ...
      // Get reference space
      const referenceSpace = await arSession.requestReferenceSpace(
        "local-floor"
      );
  ...
  const renderModel3D = (view: XRView, modelMatrix: any) => {
    ...
    const vertices = new Float32Array([
      ...
      -0.1, -0.1, 0.1, 1.0, 0.0, 1.0, 1.0, 0.1, -0.1, 0.1, 1.0, 0.0, 1.0, 1.0,
      0.1, 0.1, 0.1, 1.0, 0.0, 1.0, 1.0, -0.1, 0.1, 0.1, 1.0, 0.0, 1.0, 1.0,
      // Back face
      -0.1, -0.1, -0.1, 0.5, 0.0, 0.5, 1.0, 0.1, -0.1, -0.1, 0.5, 0.0, 0.5, 1.0,
      0.1, 0.1, -0.1, 0.5, 0.0, 0.5, 1.0, -0.1, 0.1, -0.1, 0.5, 0.0, 0.5, 1.0,
    ]);
  ...
  const renderImage = (view: XRView, modelMatrix: any) => {
    ...
    const vertices = new Float32Array([
      -0.2, -0.15, 0, 0.0, 1.0, 0.0, 0.8, 0.2, -0.15, 0, 0.0, 1.0, 0.0, 0.8,
      0.2, 0.15, 0, 0.0, 1.0, 0.0, 0.8, -0.2, 0.15, 0, 0.0, 1.0, 0.0, 0.8,
    ]);
  ...
  const renderVideo = (view: XRView, modelMatrix: any) => {
    ...
    const vertices = new Float32Array([
      -0.25, -0.15, 0, 0.0, 0.0, 1.0, 0.8, 0.25, -0.15, 0, 0.0, 0.0, 1.0, 0.8,
      0.25, 0.15, 0, 0.0, 0.0, 1.0, 0.8, -0.25, 0.15, 0, 0.0, 0.0, 1.0, 0.8,
    ]);
  ...
  const renderText = (view: XRView, modelMatrix: any) => {
    ...
    const vertices = new Float32Array([
      -0.15, -0.05, 0, 1.0, 1.0, 0.0, 0.9, 0.15, -0.05, 0, 1.0, 1.0, 0.0, 0.9,
      0.15, 0.05, 0, 1.0, 1.0, 0.0, 0.9, -0.15, 0.05, 0, 1.0, 1.0, 0.0, 0.9,
    ]);
  ...
  const renderDefault = (view: XRView, modelMatrix: any) => {
    ...
    const vertices = new Float32Array([
      -0.1, -0.1, 0, 0.5, 0.5, 0.5, 0.7, 0.1, -0.1, 0, 0.5, 0.5, 0.5, 0.7, 0.1,
      0.1, 0, 0.5, 0.5, 0.5, 0.7, -0.1, 0.1, 0, 0.5, 0.5, 0.5, 0.7,
    ]);
          ...
          {isARSupported ? (
            <div className="bg-white rounded-xl shadow-lg p-6 max-w-md mx-auto">
              ...
              <p className="text-gray-600 mb-6">
                Presiona para abrir la cámara y colocar "
                <strong>{asset.name}</strong>" en tu espacio real
              </p>
              <button
        ...
        <div className="fixed bottom-4 left-4 right-4 z-50">
          <div className="bg-black bg-opacity-80 rounded-lg p-4">
            ...
            <p className="text-white text-center text-xs mt-2">
              Toca la pantalla para reposicionar • Usa los controles para mover
              y ajustar
            </p>
          </div>
        </div>
