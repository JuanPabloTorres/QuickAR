      ...
      (set, get) => ({
        ...
        initializeSession: async () => {
          ...
          if ("xr" in navigator && navigator.xr) {
            try {
              // Check AR support
              supported.immersiveAR = await navigator.xr.isSessionSupported(
                "immersive-ar"
              );
              console.log("✅ Immersive AR supported:", supported.immersiveAR);

              // Check VR support
              supported.immersiveVR = await navigator.xr.isSessionSupported(
                "immersive-vr"
              );
        ...
        enterAR: async () => {
          ...
          try {
            ...
            const errorMessage =
              error instanceof Error ? error.message : "AR session failed";
        ...
        enterVR: async () => {
          ...
          try {
            ...
            const errorMessage =
              error instanceof Error ? error.message : "VR session failed";
...
export const useCurrentExperience = () =>
  useARStore((state) => state.currentExperience);
...
export const useAssetState = (assetId: string) =>
  useARStore((state) => state.assetStates[assetId]);
