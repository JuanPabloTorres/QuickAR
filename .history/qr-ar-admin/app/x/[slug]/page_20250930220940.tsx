...
const ARViewer: React.FC<{ experience: ExperienceDto }> = ({ experience }) => {
  ...
  const renderAsset = (asset: AssetDto) => {
    switch (asset.kind) {
      ...
      case 'message':
        return (
          <div className="ar-viewer flex items-center justify-center bg-gradient-to-br from-purple-600 to-blue-600">
    ...
    <div ref={containerRef} className="space-y-4">
      ...
      {experience.assets.length > 1 && (
        <div className="flex items-center justify-center space-x-4">
          <button
            onClick={() => {
              const newIndex = currentAssetIndex > 0 ? currentAssetIndex - 1 : experience.assets.length - 1;
          ...
          <button
            onClick={() => {
              const newIndex = currentAssetIndex < experience.assets.length - 1 ? currentAssetIndex + 1 : 0;
              setCurrentAssetIndex(newIndex);
      ...
      <div className="glass rounded-lg p-4 text-center">
        <h3 className="text-xl font-semibold text-white mb-2">{currentAsset.name}</h3>
        <p className="text-blue-200 text-sm">
          {experience.title} • {currentAsset.kind === 'model3d' ? 'Modelo 3D' : 
           currentAsset.kind === 'image' ? 'Imagen' :
           currentAsset.kind === 'video' ? 'Video' : 'Mensaje'}
        </p>
