              ...
              <div className="min-h-screen bg-gradient-to-br from-purple-900/20 via-blue-900/10 to-gray-900/20 rounded-2xl p-8">
                <div className="text-center mb-8">
                  ...
                  <p className="text-gray-300">
                    Explora las experiencias con cubos AR interactivos. Cada cubo muestra el primer asset de la experiencia.
                  </p>
                </div>
                
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-8">
                  {filteredAndSortedExperiences.map((experience, index) => {
                    // Get the first asset or create a fallback
                    const firstAsset = experience.assets[0];
                    const assetType = firstAsset 
                      ? (firstAsset.kind === "message" ? "text" : firstAsset.kind)
                      : "text";
                    
                    return (
                      <div key={experience.id} className="flex flex-col items-center space-y-4">
                        <ExperienceCube
                          assetType={assetType as any}
                          assetUrl={firstAsset?.url || undefined}
                          assetContent={firstAsset?.text || experience.title}
