    ...
    const detectCapabilities = async () => {
      try {
        ...
        if (isSecureContext && "xr" in navigator) {
          try {
            supportsWebXR =
              (await (navigator as any).xr?.isSessionSupported(
                "immersive-ar"
              )) || false;
          } catch (error) {
        ...
        const canViewAR =
          supportsWebXR || supportsSceneViewer || supportsQuickLook;
 ...
 */
export function getRecommendedARMode(
  capabilities: ARCapabilities
): "webxr" | "scene-viewer" | "quick-look" | "3d-viewer" | "none" {
