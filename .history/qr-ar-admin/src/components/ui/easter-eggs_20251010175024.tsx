...
export function MotivationalQuote() {
  ...
  return (
    <div className="fixed bottom-4 right-4 max-w-xs opacity-80 hover:opacity-100 transition-opacity z-40">
      <FuturisticCard variant="glass" className="cursor-pointer hover:scale-105 transition-transform">
        <FuturisticCardContent className="p-4">
          <div className="flex items-center space-x-2">
            <div className="text-2xl animate-pulse">{currentQuote.emoji}</div>
            <p className="text-sm text-white font-manrope">{currentQuote.text}</p>
          </div>
        </FuturisticCardContent>
      </FuturisticCard>
    </div>
...
export function FunFact({ experiences }: { experiences: any[] }) {
  ...
  useEffect(() => {
    ...
    const facts = [
      ...
      `Tus experiencias han sido vistas ${experiences.reduce((sum, exp) => sum + (exp.viewCount || 0), 0)} veces`,
