...
const TextRenderer: React.FC<TextRendererProps> = ({
  ...
  const [renderMode, setRenderMode] = useState<"plain" | "html" | "markdown">(
    "plain"
  );
    ...
    const loadTextContent = async () => {
      try {
        ...
        if (content.mime?.includes("html") || content.ext === "html") {
          ...
          content.mime?.includes("text/plain") ||
          content.ext === "txt"
        ) {
          mode = "plain";
        } else {
        ...
        setError(
          err instanceof Error
            ? err.message
            : "Error desconocido al cargar el texto"
        );
        setIsLoading(false);
      ...
      <div className="absolute top-2 right-2 z-10">
        <div className="flex gap-2">
          ...
          <button
            ...
            onClick={() => {
              const modes: Array<"plain" | "html" | "markdown"> = [
                "plain",
                "html",
                "markdown",
              ];
