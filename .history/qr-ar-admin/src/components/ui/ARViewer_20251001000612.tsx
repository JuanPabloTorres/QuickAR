...
interface TouchGestures {
  scale: num          <div className="ar-instructions ar-expanded">
            <div>🤏 Pellizca para escalar</div>
            <div>👆 Arrastra para mover</div>
            <div>👆👆 Doble toque para animar</div>
            <div>🔊 Audio y vibración habilitados</div>
            <div>🌀 Rotación automática activa</div>
...
const SimpleARViewer: React.FC<{
  ...
  }, [overlayVisible, isVideoReady, hasEntryAnimation, assetName, onTrackEvent]);
    ...
    <div className="ar-fullscreen">
      ...
      {overlayVisible && isVideoReady && !error && (
        <div className="ar-overlay">
          <div 
            ref={overlayRef}
            className={`ar-content-wrapper ar-interactive ${isAnimating ? 'ar-bouncing' : ''}`}
            style={{
              transform: `
                translate(${gestures.position.x}px, ${gestures.position.y}px) 
                scale(${gestures.scale}) 
                rotate(${gestures.rotation}deg)
              `,
              transition: gestures.isDragging || gestures.isScaling ? 'none' : 'transform 0.3s ease',
            }}
        ...
        <div className="ar-footer-content ar-footer-content-auto">
          <div className="ar-controls-grid">
            ...
            <button
              ...
              className={`ar-toggle-btn ${autoRotate ? 'ar-active-btn' : 'ar-inactive-btn'}`}
            >
            ...
            <button
              onClick={toggleSound}
              className={`ar-toggle-btn ${soundEnabled ? 'ar-sound-on' : 'ar-sound-off'}`}
            >
