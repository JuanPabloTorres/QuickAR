...
const Enhanced3DViewer: React.FC<Enhanced3DViewerProps> = ({
  ...
  const handleARClick = useCallback(() => {
    if (!state.supportsAR) {
      ...
      alert(
        "Realidad Aumentada no está disponible en este dispositivo o navegador."
      );
      return;
    }
  ...
  if (showAR) {
    return (
      <div
        className={`relative bg-gradient-to-br from-purple-900 via-blue-900 to-indigo-900 rounded-xl overflow-hidden ${className}`}
      >
        {/* AR Background */}
