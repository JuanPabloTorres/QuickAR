...
const ARViewer = dynamic(() => import("./ARViewer"), {
  ssr: false,
  loading: () => <div className="animate-pulse bg-slate-200 rounded-xl h-96 flex items-center justify-center">Cargando visor AR...</div>,
});
...
const ModernAssetCarousel: React.FC<ModernAssetCarouselProps> = ({
  ...
  const getAssetKind = useCallback((asset: AssetDto): AssetKind => {
    ...
    if (mime.startsWith("image/") || ["jpg", "jpeg", "png", "webp", "avif", "gif"].includes(ext)) {
      return "image";
    }
  ...
  const renderAssetContent = useCallback((asset: AssetDto) => {
        ...
        return (
          <div className="relative h-full bg-gradient-to-br from-blue-100 to-purple-200 rounded-xl flex items-center justify-center p-8">
            <div className="text-center max-w-md">
              ...
              <h3 className="text-xl font-bold text-slate-800 mb-4">{asset.name}</h3>
              {asset.text && (
                <div className="bg-white/80 backdrop-blur-sm rounded-lg p-4 text-slate-700">
                  <p className="whitespace-pre-wrap text-sm leading-relaxed">{asset.text}</p>
                </div>
              )}
            </div>
          </div>
      ...
      <div className="mt-6 bg-white/80 backdrop-blur-sm rounded-xl p-6 shadow-lg border border-slate-200">
        ...
        <div className="flex items-center gap-4 text-sm text-slate-600">
          <span className="flex items-center gap-1">
            ...
            {getAssetKind(currentAsset).charAt(0).toUpperCase() + getAssetKind(currentAsset).slice(1)}
          </span>
