...
const Enhanced3DViewer: React.FC<Enhanced3DViewerProps> = ({
  ...
  const checkARSupport = useCallback(() => {
    ...
    const supportsAR = isSecure && (hasWebXR || 'DeviceOrientationEvent' in window);
  ...
  }, [state.modelViewerLoaded, assetUrl, asset.name, asset.id, state.supportsAR, onTrackEvent]);
  ...
  if (state.error) {
    return (
      <div className={`relative bg-gradient-to-br from-red-50 to-red-100 rounded-xl p-8 ${className}`}>
  ...
  return (
    <div className={`relative bg-gradient-to-br from-slate-50 to-slate-100 rounded-xl overflow-hidden ${className}`}>
        ...
        <div className="absolute inset-0 flex items-center justify-center bg-slate-900/5 backdrop-blur-sm z-10">
          <div className="text-center">
            ...
            <p className="text-sm font-medium text-slate-600">
              {!state.modelViewerLoaded ? "Inicializando visor 3D..." : "Cargando modelo..."}
            </p>
          </div>
        </div>
      )}
      ...
      <div 
        ...
        style={{ 
          // Ensure proper touch handling for 3D controls
          touchAction: 'pan-y',
          userSelect: 'none'
        }}
      />
