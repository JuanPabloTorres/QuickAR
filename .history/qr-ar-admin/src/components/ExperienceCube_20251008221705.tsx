...
import { AssetRenderer, AssetPreview } from './AssetRenderer'
import { 
  getPrimaryAsset, 
  hasARCapabilities, 
...
export function ExperienceCube({
  ...
  const handleARPreview = useCallback((e: React.MouseEvent) => {
    e.preventDefault()
    e.stopPropagation()
            ...
            <div className="min-w-0 flex-grow">
              ...
              {showStats && (
                <div className="flex items-center space-x-4 mt-2 text-xs text-gray-500">
                  ...
                  {!experience.isActive && <span className="text-red-500">Inactivo</span>}
                </div>
              )}
            </div>
        ...
        {showStats && (
          <div className="flex items-center justify-between text-sm text-gray-500 mb-4">
            ...
            <div className="flex items-center space-x-2">
              {stats.has3DModels && <span title="Modelos 3D">🎯</span>}
              {stats.hasImages && <span title="Imágenes">🖼️</span>}
              {stats.hasVideos && <span title="Videos">📹</span>}
              {stats.hasTexts && <span title="Textos">📝</span>}
            </div>
          </div>
        )}
...
export function ExperienceGrid({ 
  ...
  return (
    <div className={cn(
      "grid gap-6",
      variant === 'list' ? 'grid-cols-1' : Object.entries(gridCols)
