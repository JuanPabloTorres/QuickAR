...
import { ARState, ARCapabilities, ARAsset } from '@/types'
import { 
  getARCapabilities,
...
export function ArProvider({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    const initializeAR = async () => {
      try {
        ...
        setState(current => updateARState(current, {
  ...
  // Iniciar sesión AR
  const startARSession = useCallback(async (asset: ARAsset): Promise<boolean> => {
    try {
      ...
      setState(current => updateARState(current, {
        isLoading: false,
        error: error instanceof Error ? error.message : 'Error al iniciar AR'
      }))
      return false
    }
  }, [capabilities])
...
export function ArClient({
      ...
      <div className={`p-4 bg-yellow-50 border border-yellow-200 rounded-lg ${className}`}>
        ...
        {showDetails && capabilities && (
          <div className="mt-3 p-3 bg-yellow-100 rounded text-xs text-yellow-800">
            ...
            <div>World Tracking: {capabilities.worldTracking ? '✅' : '❌'}</div>
          </div>
        )}
      </div>
    )
  ...
  return (
    <div className={`ar-client ${className}`}>
      ...
      {!state.isActive && !state.isLoading && asset && asset.type === 'model3d' && (
...
export function useARSession(asset?: ARAsset) {
  ...
  const update = useCallback((updates: Partial<ARState>) => {
    if (isThisAssetActive) {
