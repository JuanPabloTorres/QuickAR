...
import { AR3DSettings, ARAsset } from "@/types";
...
export function AssetRenderer({
  ...
  const renderModel3D = () => {
    ...
    const modelViewerAttributes = generateModelViewerAttributes(
      asset,
      settings
    );
    ...
    return (
      <div className="relative w-full h-64 md:h-96">
        ...
        <model-viewer
          ...
          {...(Object.fromEntries(
            Object.entries(
              generateModelViewerAttributes(asset, {
                ...settings,
                enableInteraction:
                  showControls && (settings?.enableInteraction ?? true),
              })
            )
              .filter(([_, value]) =>
    ...
    return (
      <div className="relative w-full">
        <video
          ...
          className={cn("w-full h-auto max-h-96 rounded-lg", className)}
  ...
  const renderText = () => {
    ...
    return (
      ...
      >
        <div className="whitespace-pre-wrap text-gray-800">{asset.content}</div>
      </div>
    );
  };
