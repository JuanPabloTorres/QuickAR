...
function TextAsset({ asset, onLoad, onInteraction }: AssetComponentProps) {
  ...
  const transformStyle = {
    transform: `translate3d(${asset.position?.x || 0}px, ${
      asset.position?.y || 0
    }px, ${asset.position?.z || 0}px)`,
  };
...
function ImageAsset({
  ...
  const transformStyle = {
    transform: `translate3d(${asset.position?.x || 0}px, ${
      asset.position?.y || 0
    }px, ${asset.position?.z || 0}px) rotateX(${
      asset.rotation?.x || 0
    }deg) rotateY(${asset.rotation?.y || 0}deg) rotateZ(${
      asset.rotation?.z || 0
    }deg)`,
  };
  ...
  return (
    <div
      className="ar-image-asset cursor-pointer hover:scale-105 transition-transform"
      onClick={onInteraction}
      style={transformStyle}
    >
      {asset.url && (
...
function Model3DAsset({
  ...
  return (
    <div
      className="ar-model-container"
      onClick={onInteraction}
      style={transformStyle}
    >
      {asset.url && (
        <div className="ar-model-viewer bg-gradient-to-br from-blue-50 to-indigo-100 flex items-center justify-center border-2 border-dashed border-blue-300">
          <div className="text-center p-8">
            <div className="text-4xl mb-4">🧊</div>
            <h3 className="text-lg font-medium text-gray-900 mb-2">
              {asset.name}
            </h3>
            <p className="text-sm text-gray-600 mb-4">Modelo 3D (Simulado)</p>
