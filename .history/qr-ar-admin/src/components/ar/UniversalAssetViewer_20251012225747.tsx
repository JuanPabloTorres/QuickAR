...
// Temporary inline resolver until import is fixed
async function resolveAssetUrl(assetUrl: string | undefined, assetName: string, assetId: string, assetType: string): Promise<string> {
  // If it's already a valid server URL, use it
  if (assetUrl && !assetUrl.startsWith("blob:") && assetUrl.startsWith("/uploads/")) {
    return assetUrl;
  }

  // For web content, return URL as-is if valid HTTP(S)
  ...
  // Fallback URLs based on asset type
  const fallbackUrls: Record<string, string> = {
    model3d: "/uploads/models/1760250740_39d3d473.glb", // Known working model
    image: "/uploads/images/default-image.jpg",
    video: "/uploads/videos/default-video.mp4",
    webcontent: "https://example.com"
  };

  return fallbackUrls[assetType] || "/uploads/default-asset";
}
    ...
    const resolveAssetUrl = async () => {
      ...
      try {
        ...
        if (asset.assetType === "message") {
          ...
          resolved = await resolveAssetUrl(
            asset.assetUrl,
            asset.name,
            asset.id,
            asset.assetType
