  ...
  return (
    <div className={`w-full h-full relative ${className}`}>
      <model-viewer
        ...
        // @ts-ignore - model-viewer events
        onArStatus={handleARStatusChange}
      />
      ...
      <div className="absolute top-4 left-4 bg-black bg-opacity-50 text-white p-2 rounded text-sm max-w-48">
        ...
        <div className="text-xs opacity-75 mt-1 flex items-center justify-between">
          <span>{src.split("/").pop()?.split("_").pop()?.toUpperCase() || "GLB"}</span>
          {arReady && (
            <span className="text-green-300 ml-2">🎯 AR Ready</span>
          )}
        </div>
      </div>
