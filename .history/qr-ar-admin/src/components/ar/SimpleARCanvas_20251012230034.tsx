...
export default function SimpleARCanvas({
  ...
  const handleARActivation = async () => {
    ...
    try {
      ...
      console.log(`🔍 DOM contains ${allModelViewers.length} model-viewer elements:`, 
      ...
      while (!modelViewer && attempts < maxAttempts) {
        ...
        for (const selector of selectors) {
          ...
          if (modelViewer) {
            console.log(`✅ Found model-viewer with selector "${selector}" on attempt ${attempts + 1}:`, {
              element: modelViewer,
        ...
        <button
          onClick={() => {
            ...
            const debugInfo = {
              ...
              currentAssetIndex,
              currentAsset: experience.assets[Math.min(currentAssetIndex, experience.assets.length - 1)],
            ...
            const mv = document.querySelector('.universal-model-viewer') || document.querySelector('model-viewer');
            if (mv) {
              console.log("Found element, adding red border for identification");
