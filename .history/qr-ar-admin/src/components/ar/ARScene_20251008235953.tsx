...
export function ARScene({
  ...
  useEffect(() => {
    const initializeARScene = async () => {
      try {
        ...
        const errorMessage =
          err instanceof Error ? err.message : "Error desconocido en AR";
  ...
  const handleExperienceError = useCallback(
    (error: string) => {
      setError(error);
      setLoadingState("error");
      ...
      {loadingState === "error" && (
        <div className="absolute inset-0 flex items-center justify-center bg-red-900/50 backdrop-blur-sm">
          <div className="text-center text-white p-8">
            ...
            <p className="text-sm opacity-75 mb-4">
              {error ||
                "No se pudo inicializar la experiencia de realidad aumentada"}
            </p>
            <button
