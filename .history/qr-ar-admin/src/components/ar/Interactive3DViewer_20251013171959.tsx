...
export default function Interactive3DViewer({
  ...
  // Update asset visibility when selectedIndex changes
  useEffect(() => {
    const groups = assetGroupsRef.current;
    if (groups && groups.length > 0) {
      groups.forEach((group, index) => {
        group.visible = index === selectedIndex;
      });
      console.log(`🎯 Showing asset ${selectedIndex + 1} of ${groups.length}`);
    }
  }, [selectedIndex]);
  ...
  useEffect(() => {
    ...
    import("three").then(async (THREE) => {
      ...
      // OrbitControls - Natural camera movement (works with touch and mouse)
      ...
      // Mobile-friendly touch controls (automatically enabled by OrbitControls)
      controls.touches = {
      ...
      experience.assets.forEach((asset, index) => {
        ...
        switch (asset.assetType) {
          ...
          case "image": {
            if (asset.assetUrl) {
              ...
              textureLoader.load(imageUrl, (texture) => {
                ...
                // Modern floating image with subtle white border
                const imageGroup = new THREE.Group();

                // Thin white border/frame (modern and clean)
                const borderThickness = 0.05;
                const borderDepth = 0.03;
                ...
                const leftBorder = new THREE.Mesh(
                  new THREE.BoxGeometry(borderThickness, height, borderDepth),
                  borderMaterial
                );
                ...
                // Image plane with enhanced material
                ...
                imagePlane.position.z = borderDepth / 2;

                // Optional: Subtle backplate for depth
                const backplate = new THREE.Mesh(
                  new THREE.PlaneGeometry(width + 0.02, height + 0.02),
                  new THREE.MeshStandardMaterial({
                    color: 0xf8f8f8, // Very light gray
                    roughness: 0.8,
                    metalness: 0.0,
                  })
                );
                ...
                imageGroup.castShadow = true;
                [topBorder, bottomBorder, leftBorder, rightBorder, imagePlane].forEach(
                  (obj) => (obj.castShadow = true)
                );
          ...
          case "message": {
            // Modern floating card with text
            const signGroup = new THREE.Group();
            const signWidth = 2.0;
            const signHeight = 1.4;

            // Main card background (light blue/cyan)
            const cardBoard = new THREE.Mesh(
              new THREE.BoxGeometry(signWidth, signHeight, 0.06),
              new THREE.MeshStandardMaterial({
                color: 0x93c5fd, // Light blue (blue-300)
                roughness: 0.2,
                metalness: 0.1,
              })
            );

            // Elegant white border/frame
            const frame = new THREE.Mesh(
              new THREE.BoxGeometry(signWidth + 0.08, signHeight + 0.08, 0.04),
              new THREE.MeshStandardMaterial({
            ...
            // Add text to the card using canvas texture
            if (asset.assetContent) {
              ...
              if (context) {
                ...
                // Rounded rectangle for inner card
                const radius = 20;
                context.beginPath();
                context.moveTo(padding + radius, padding);
                context.lineTo(padding + innerWidth - radius, padding);
                ...
                context.lineTo(
                  padding + innerWidth,
                  padding + innerHeight - radius
                );
                context.quadraticCurveTo(
                  padding + innerWidth,
                  padding + innerHeight,
                  padding + innerWidth - radius,
                  padding + innerHeight
                );
                context.lineTo(padding + radius, padding + innerHeight);
                context.quadraticCurveTo(
                  padding,
                  padding + innerHeight,
                  padding,
                ...
                const textPlane = new THREE.Mesh(
                  new THREE.PlaneGeometry(signWidth - 0.02, signHeight - 0.02),
                ...
                textPlane.position.z = 0.035; // In front of the board
                signGroup.add(textPlane);
              }
            }

            // Slim elegant stand
            const standHeight = signHeight + 0.8;
            const stand = new THREE.Mesh(
              new THREE.CylinderGeometry(0.05, 0.08, standHeight, 16),
              new THREE.MeshStandardMaterial({
                color: 0xf0f0f0, // Light gray/silver
                roughness: 0.4,
                metalness: 0.6,
              })
            );
            stand.position.y = -signHeight / 2 - 0.4;

            signGroup.add(frame, cardBoard, stand);
            signGroup.castShadow = true;
            [frame, cardBoard, stand].forEach((p) => (p.castShadow = true));
      ...
      const animate = () => {
        ...
        assetGroups.forEach((group, index) => {
          // Control visibility - only show selected asset
          const isSelected = index === selectedIndex;
          group.visible = isSelected;

          // Animate only visible asset
          if (isSelected) {
            // Gentle floating
  ...
  return (
    <div className="relative w-full h-screen bg-gradient-to-br from-gray-50 via-gray-100 to-gray-200">
        ...
        <div className="absolute bottom-8 left-1/2 -translate-x-1/2 z-20">
          <div className="flex items-center gap-4 bg-white/95 backdrop-blur-md rounded-full px-6 py-3 shadow-xl border border-gray-200">
            <button
              ...
              className="p-2 hover:bg-gray-100 active:bg-gray-200 rounded-full transition-all text-gray-700 touch-manipulation"
      ...
      {/* Zoom Controls - Subtle buttons (mobile-friendly) */}
      <div className="absolute right-4 bottom-32 z-20 flex flex-col gap-2">
        <button
          onClick={handleZoomIn}
          className="p-3 bg-black/60 hover:bg-black/80 active:bg-black/90 backdrop-blur-sm rounded-full transition-all text-white shadow-lg touch-manipulation"
          aria-label="Zoom in"
          title="Acercar"
        >
          <svg
