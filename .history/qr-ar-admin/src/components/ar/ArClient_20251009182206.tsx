...
export function ArClient({ experience, onBack, className = '' }: ArClientProps) {
  ...
  useEffect(() => {
    const handleKeyDown = (e: KeyboardEvent) => {
      switch (e.key) {
        case 'ArrowLeft':
          e.preventDefault();
          handleAssetChange(currentAssetIndex > 0 ? currentAssetIndex - 1 : experience.assets.length - 1);
          break;
        case 'ArrowRight':
          e.preventDefault();
          handleAssetChange(currentAssetIndex < experience.assets.length - 1 ? currentAssetIndex + 1 : 0);
          break;
        case 'Escape':
          if (isFullscreen) {
  ...
  if (isLoading) {
    return (
      <div className={`ar-container bg-gradient-to-br from-slate-950 via-brand-950 to-qr-950 flex items-center justify-center ${className}`}>
        <div className="text-center text-white">
          <div className="animate-ar-pulse text-6xl mb-6">🔄</div>
          <div className="text-xl font-semibold mb-2">Iniciando experiencia AR</div>
    ...
    return (
      <div className={`ar-container bg-gradient-to-br from-slate-950 via-brand-950 to-qr-950 flex items-center justify-center ${className}`}>
        <div className="text-center text-white max-w-md">
          ...
          <div className="text-xl font-semibold mb-4">Error en la experiencia AR</div>
          <div className="text-sm text-red-200 mb-6">
            {error}
          </div>
  ...
  return (
    <div className={`ar-container bg-gradient-to-br from-slate-950 via-brand-950 to-qr-950 ${className}`}>
      {/* AR Scene */}
