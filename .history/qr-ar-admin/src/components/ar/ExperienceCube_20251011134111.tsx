...
export function ExperienceCube({
  ...
  const currentAsset = experience.assets[currentAssetIndex] || experience.assets[0];
    ...
    >
      ...
      {arScanActive && (
        <div className="absolute inset-0 pointer-events-none z-10">
          <div className="w-full h-full border border-sky-400/30 relative">
            {/* Scanning lines */}
            <div className={`absolute w-full h-0.5 bg-gradient-to-r from-transparent via-sky-400 to-transparent animate-pulse ${glitchEffect ? 'opacity-100' : 'opacity-60'}`} 
                 style={{ top: '25%', animationDuration: '2s' }} />
            <div className={`absolute w-full h-0.5 bg-gradient-to-r from-transparent via-green-400 to-transparent animate-pulse ${glitchEffect ? 'opacity-60' : 'opacity-100'}`} 
                 style={{ top: '50%', animationDuration: '2.5s', animationDelay: '0.5s' }} />
            <div className={`absolute w-full h-0.5 bg-gradient-to-r from-transparent via-purple-400 to-transparent animate-pulse ${glitchEffect ? 'opacity-100' : 'opacity-40'}`} 
                 style={{ top: '75%', animationDuration: '3s', animationDelay: '1s' }} />
            
            {/* Corner markers */}
            <div className="absolute top-4 left-4 w-8 h-8 border-t-2 border-l-2 border-sky-400 opacity-80" />
            <div className="absolute top-4 right-4 w-8 h-8 border-t-2 border-r-2 border-sky-400 opacity-80" />
            <div className="absolute bottom-4 left-4 w-8 h-8 border-b-2 border-l-2 border-sky-400 opacity-80" />
            <div className="absolute bottom-4 right-4 w-8 h-8 border-b-2 border-r-2 border-sky-400 opacity-80" />
          </div>
        </div>
      )}
      ...
      <div className="relative">
        <div
          ref={cubeRef}
          className={`experience-cube w-80 h-80 mx-auto cursor-grab select-none relative transition-all duration-500 ${
          ...
          style={{
            ...
            transition: isInteracting ? "none" : "transform 0.3s ease-out, scale 0.5s ease-out",
            filter: glitchEffect ? 'hue-rotate(180deg) saturate(150%)' : 'none',
          }}
        >
          {/* Main face - Current Asset with enhanced effects */}
          <div
            className={`cube-face absolute w-80 h-80 backdrop-blur-md flex items-center justify-center border border-white/10 ${
              isHovered ? 'bg-gradient-to-br from-sky-500/20 via-purple-500/15 to-green-500/20' 
                        : 'bg-gradient-to-br from-sky-900/30 via-purple-900/20 to-green-900/30'
            } transition-all duration-500`}
            style={{
              transform: "translateZ(140px)",
              boxShadow: isHovered ? '0 0 40px rgba(56, 189, 248, 0.3), inset 0 0 40px rgba(168, 85, 247, 0.1)' : 'none',
            }}
          >
            <div className="w-72 h-72 relative">
              <AssetRenderer asset={currentAsset} className="w-full h-full rounded-lg" />
              {/* AR Frame overlay */}
              <div className="absolute inset-0 border-2 border-sky-400/40 rounded-lg">
          ...
          <div
            className="cube-face absolute w-80 h-80 bg-gradient-to-br from-slate-900/90 via-purple-900/30 to-slate-900/90 backdrop-blur-md flex items-center justify-center border border-white/10"
            style={{
          ...
          {experience.assets.slice(1, 5).map((asset, index) => (
            <div
              key={asset.id}
              className={`cube-face absolute w-80 h-80 backdrop-blur-md flex items-center justify-center cursor-pointer border border-white/10 transition-all duration-300 ${
                currentAssetIndex === index + 1 
                  ? 'bg-gradient-to-br from-green-500/30 via-blue-500/20 to-purple-500/30' 
                  : 'bg-gradient-to-br from-slate-900/40 via-purple-900/20 to-slate-900/40 hover:from-sky-500/20 hover:via-purple-500/15 hover:to-green-500/20'
              }`}
              style={{
                transform: `rotateY(${90 * (index + 1)}deg) translateZ(140px)`,
                boxShadow: currentAssetIndex === index + 1 ? '0 0 30px rgba(34, 197, 94, 0.3)' : 'none',
              }}
              onClick={() => onAssetChange?.(index + 1)}
            >
              <div className="w-60 h-60 flex items-center justify-center relative">
                <AssetRenderer asset={asset} className="w-full h-full rounded-lg" />
                {/* Selection indicator */}
                {currentAssetIndex === index + 1 && (
                  <div className="absolute inset-0 border-2 border-green-400 rounded-lg animate-pulse" />
                )}
                {/* Asset number badge */}
                <div className="absolute top-2 left-2 w-6 h-6 bg-purple-600/80 text-white text-xs rounded-full flex items-center justify-center font-bold">
                  {index + 2}
                </div>
              </div>
            </div>
        ...
        {/* Holographic base */}
        <div className="absolute bottom-0 left-1/2 transform -translate-x-1/2 translate-y-4 w-96 h-4 bg-gradient-to-r from-transparent via-sky-400/20 to-transparent rounded-full blur-sm" />
        <div className="absolute bottom-0 left-1/2 transform -translate-x-1/2 translate-y-6 w-80 h-2 bg-gradient-to-r from-transparent via-purple-400/30 to-transparent rounded-full blur-md" />
      </div>
            ...
            <div className="flex items-center justify-center space-x-4 text-xs text-slate-400 font-manrope">
              <span className="flex items-center space-x-1">
                <div className={`w-2 h-2 rounded-full ${
                  currentAsset.assetType === 'image' ? 'bg-green-400' :
                  currentAsset.assetType === 'video' ? 'bg-red-400' :
                  currentAsset.assetType === 'model3d' ? 'bg-blue-400' :
                  'bg-purple-400'
                }`} />
                <span>{currentAsset.assetType.toUpperCase()}</span>
              </span>
              <span>{currentAssetIndex + 1} / {experience.assets.length}</span>
            </div>
          </div>
        </div>
      )}
      ...
      <div className="flex justify-center mt-6">
        <div className={`flex items-center space-x-2 px-4 py-2 rounded-full backdrop-blur-md transition-all duration-300 ${
          arScanActive 
      ...
      <div className="text-center mt-4 text-sm text-slate-400">
        <div className="flex items-center justify-center space-x-2">
          <QuickArLogo size={16} />
          <span className="font-manrope">Arrastra para rotar • Hover para escanear AR</span>
          {experience.assets.length > 1 && (
            <>
              <span>•</span>
