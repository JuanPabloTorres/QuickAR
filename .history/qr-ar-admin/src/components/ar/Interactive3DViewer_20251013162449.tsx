...
export default function Interactive3DViewer({
  ...
  const handleZoomIn = useCallback(() => {
    if (controlsRef.current) {
      ...
      camera.position.addScaledVector(direction, 1); // Move 1 unit closer
    }
  }, []);
  ...
  const handleZoomOut = useCallback(() => {
    if (controlsRef.current) {
      ...
      const direction = new (window as any).THREE.Vector3();
      camera.getWorldDirection(direction);
      camera.position.addScaledVector(direction, -1); // Move 1 unit away
    }
  }, []);
  ...
  useEffect(() => {
    ...
    import("three").then(async (THREE) => {
      ...
      // Subtle 3D axes helper for spatial reference
      const axesHelper = new THREE.AxesHelper(1.5);
      axesHelper.position.set(0, 0.5, 0);
      // Make axes more subtle and translucent
      (axesHelper.material as any).opacity = 0.4;
      (axesHelper.material as any).transparent = true;
