...
export function ExperienceCube({
  ...
  // AR scanning animation - only for non-3D assets
  useEffect(() => {
    if (isHovered && currentAsset.assetType !== 'model3d') {
      setArScanActive(true);
    } else {
      setArScanActive(false);
      setGlitchEffect(false);
    }
  }, [isHovered, currentAsset.assetType]);
  ...
  return (
    ...
    >
      ...
      <div className="relative">
        ...
        {/* Minimal holographic base - only for non-3D assets */}
        {currentAsset.assetType !== 'model3d' && (
          <div className="absolute bottom-0 left-1/2 transform -translate-x-1/2 translate-y-4 w-64 sm:w-80 h-2 bg-gradient-to-r from-transparent via-sky-400/20 to-transparent rounded-full blur-sm" />
        )}
      </div>
