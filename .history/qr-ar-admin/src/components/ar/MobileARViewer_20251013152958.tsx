...
export default function MobileARViewer({
  ...
  const handlePrevious = () => {
    const prevIndex = selectedIndex === 0 ? experience.assets.length - 1 : selectedIndex - 1;
  ...
  const getAssetUrl = (asset: Asset): string => {
    ...
    if (asset.assetUrl.startsWith("http://") || asset.assetUrl.startsWith("https://")) {
  ...
  const canShowARButton = selectedAsset?.assetType === "model3d" && isModelViewerLoaded;
    ...
    <div className="relative w-full h-screen bg-gradient-to-br from-indigo-900 via-purple-900 to-pink-900">
      ...
      <div className="absolute bottom-0 left-0 right-0 bg-gradient-to-t from-black/80 to-transparent p-6 z-10">
        ...
        <div className="bg-white/10 backdrop-blur rounded-xl p-4 mb-4">
          <h3 className="text-white font-bold text-lg mb-1">{selectedAsset.name}</h3>
      ...
      {canShowARButton && (
        <div className="absolute left-4 top-1/2 -translate-y-1/2 z-10 max-w-xs">
          <div className="bg-black/60 backdrop-blur text-white text-xs px-3 py-2 rounded-lg">
            ...
            <p>Toca el botón "Ver en AR" para ver este modelo en tu espacio real</p>
          </div>
        </div>
      )}
    </div>
