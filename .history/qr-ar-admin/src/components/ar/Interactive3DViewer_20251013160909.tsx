...
export default function Interactive3DViewer({
  ...
  useEffect(() => {
    ...
    import("three").then(async (THREE) => {
      ...
      // Setup video texture from camera feed
      const video = videoRef.current;
      if (video && video.readyState === video.HAVE_ENOUGH_DATA) {
        ...
        scene.background = null; // Transparent until video loads
      }
      ...
      // Realistic lighting - natural and clean
      ...
      // Subtle fill lights
      const fillLight1 = new THREE.PointLight(0xffffff, 0.25, 20);
      fillLight1.position.set(-5, 3, -5);
      scene.add(fillLight1);

      const fillLight2 = new THREE.PointLight(0xffffff, 0.25, 20);
      ...
      const groundMaterial = new THREE.ShadowMaterial({
        opacity: 0.12, // Very subtle shadows
      });
      ...
      // Optional: Subtle grid for spatial reference
      const gridHelper = new THREE.GridHelper(50, 50, 0xd0d0d0, 0xe8e8e8);
      gridHelper.material.opacity = 0.12;
      gridHelper.material.transparent = true;
      gridHelper.position.y = 0.001;
      scene.add(gridHelper);

      // NO PLATFORM - Assets float at natural height
  ...
  return (
    <div className="relative w-full h-screen bg-gradient-to-br from-gray-50 via-gray-100 to-gray-200">
      ...
      <div className="absolute bottom-32 left-1/2 -translate-x-1/2 z-20 max-w-lg">
        <div className="bg-white/95 backdrop-blur-md rounded-xl p-4 text-gray-800 text-center shadow-xl border border-gray-200">
          ...
          <div className="mt-3 flex items-center justify-center gap-2 text-sm text-gray-600">
            <span className="px-3 py-1 bg-gray-100 rounded-full capitalize font-medium">
              {selectedAsset.assetType}
            </span>
