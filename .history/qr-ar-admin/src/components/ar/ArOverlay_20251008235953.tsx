...
import { ARAsset, ARExperience, LoadingState } from "@/lib/types";
          ...
          <div className="bg-black/80 text-white rounded-lg p-3 max-w-xs">
            ...
            <div className="mt-2 flex items-center space-x-3 text-xs opacity-75">
              {currentAsset.type === "model3d" && currentAsset.metadata && (
                <>
                  <span>
                    📦 {currentAsset.metadata.format?.toUpperCase() || "3D"}
                  </span>
                  {currentAsset.metadata.size && (
                    <span>
                      💾 {Math.round(currentAsset.metadata.size / 1024)}KB
                    </span>
                  )}
                </>
              )}
              ...
              {currentAsset.type === "image" && currentAsset.metadata && (
                <>
                  <span>
                    🖼️ {currentAsset.metadata.format?.toUpperCase() || "IMG"}
                  </span>
                  {currentAsset.metadata.dimensions && (
                    <span>
                      📐 {currentAsset.metadata.dimensions.width}x
                      {currentAsset.metadata.dimensions.height}
                    </span>
                  )}
                </>
              )}
              ...
              {currentAsset.type === "video" && currentAsset.metadata && (
                <>
                  <span>
                    🎥 {currentAsset.metadata.format?.toUpperCase() || "VID"}
                  </span>
                  {currentAsset.metadata.duration && (
                    <span>
                      ⏱️ {Math.round(currentAsset.metadata.duration)}s
                    </span>
                  )}
                </>
              )}
        ...
        <div className="bg-white/90 backdrop-blur-sm rounded-full px-4 py-2 shadow-lg">
          <p className="text-sm text-gray-700 text-center">
            {currentAsset?.type === "model3d" &&
              "👆 Toca y arrastra para interactuar con el modelo"}
            {currentAsset?.type === "video" &&
              "▶️ Toca para reproducir/pausar el video"}
            {currentAsset?.type === "image" &&
              "🔍 Toca para ver detalles de la imagen"}
