...
export function ExperienceCube({
  ...
  const handleAssetError = useCallback((error: string, asset: ARAsset) => {
    onError?.(`Error loading ${asset.name}: ${error}`);
  }, [onError]);

  const handleAssetClick = useCallback((asset: ARAsset, index: number) => {
        ...
        <div className="text-center p-6">
          <div className="w-16 h-16 mx-auto mb-4 rounded-full bg-gray-100 flex items-center justify-center">
            <svg width="32" height="32" fill="none" viewBox="0 0 24 24" className="text-gray-400">
              ...
              <circle cx="12" cy="16" r="0.25" stroke="currentColor" strokeWidth="2"/>
            </svg>
          </div>
          <h3 className="font-medium text-gray-700 text-sm mb-2">{experience.title}</h3>
  ...
  const primaryAsset = experience.assets[selectedAssetIndex] || experience.assets[0];
    ...
    <div className={`relative ${className}`}>
      ...
      {experience.assets.length > 1 && enableInteraction && (
        <div className="absolute bottom-3 left-3 right-3">
          <div className="flex items-center justify-between bg-black bg-opacity-50 rounded-lg px-3 py-2">
            <button
              onClick={() => setSelectedAssetIndex(Math.max(0, selectedAssetIndex - 1))}
            ...
            >
              <svg width="16" height="16" fill="none" viewBox="0 0 24 24">
                <path stroke="currentColor" strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M15 18L9 12L15 6"/>
              </svg>
            </button>
              ...
              {experience.assets.map((_, index) => (
                <button
                  ...
                  className={`w-2 h-2 rounded-full transition-colors ${
                    index === selectedAssetIndex ? 'bg-white' : 'bg-white bg-opacity-40'
                  }`}
                />
              ))}
            </div>
            ...
            <button
              onClick={() => setSelectedAssetIndex(Math.min(experience.assets.length - 1, selectedAssetIndex + 1))}
              disabled={selectedAssetIndex === experience.assets.length - 1}
              className="text-white hover:text-gray-300 disabled:text-gray-500 disabled:cursor-not-allowed"
            >
              <svg width="16" height="16" fill="none" viewBox="0 0 24 24">
                <path stroke="currentColor" strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M9 18L15 12L9 6"/>
              </svg>
            </button>
          </div>
        </div>
      ...
      <div className="absolute top-3 left-3 right-3">
        <div className="bg-black bg-opacity-50 rounded-lg px-3 py-2">
          <h3 className="text-white text-sm font-medium truncate">{experience.title}</h3>
          <p className="text-white text-xs opacity-75 truncate">{primaryAsset.name}</p>
        </div>
      </div>
    </div>
  );
  ...
  const renderGridMode = () => {
    ...
    return (
      ...
      >
        ...
        >
          {experience.assets.slice(0, gridSize * gridSize).map((asset, index) => (
            <div
              key={asset.id}
              ...
              className={`
                ...
                ${selectedAssetIndex === index ? 'ring-2 ring-blue-500 ring-offset-1' : ''}
              `}
            >
              <AssetRenderer
                asset={asset}
        ...
        <div className="absolute bottom-2 left-2 right-2">
          <div className="bg-black bg-opacity-50 rounded px-2 py-1">
            <h3 className="text-white text-xs font-medium truncate">{experience.title}</h3>
          </div>
        </div>
      </div>
    );
