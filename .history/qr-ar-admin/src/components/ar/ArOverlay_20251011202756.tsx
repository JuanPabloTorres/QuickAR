...
export function ArOverlay({
  ...
  return (
    <div className={`ar-overlay ${className}`}>
      ...
      >
        <div className="glass m-2 sm:m-4 p-3 sm:p-4 flex items-center justify-between">
          <button
            onClick={onBack}
            className="ar-navigation-button flex items-center space-x-2 text-sm"
            aria-label="Volver"
          >
            <span>←</span>
          ...
          <div className="ar-navigation-button px-2 sm:px-3 py-1 text-xs sm:text-sm">
      ...
      >
        <div className="glass m-2 sm:m-4 p-3 sm:p-4">
          {/* Asset navigation for multiple assets */}
          {experience.assets.length > 1 && (
            <div className="flex justify-center mb-3 sm:mb-4 space-x-2 sm:space-x-4">
              <button
                onClick={() => {
                  const newIndex =
                    currentAssetIndex > 0
                ...
                className="ar-navigation-button text-sm"
                aria-label="Anterior"
              >
                ◀
              </button>
          ...
          {currentAsset && (
            <div className="text-center text-white">
              <h3 className="text-base sm:text-lg font-medium mb-2">{currentAsset.name}</h3>

              {currentAsset.assetType === "message" &&
                currentAsset.assetContent && (
                  <p className="text-blue-200 text-sm leading-relaxed max-w-md mx-auto px-2">
                    {currentAsset.assetContent}
                  </p>
                )}
      ...
      <div className="absolute right-2 sm:right-4 top-1/2 transform -translate-y-1/2">
        <button
          ...
          className="ar-navigation-button w-10 h-10 sm:w-12 sm:h-12 flex items-center justify-center"
          aria-label={isVisible ? "Ocultar controles" : "Mostrar controles"}
        >
          <QuickArLogo
            size={14}
            className={`sm:w-4 sm:h-4 ${isVisible ? "opacity-100" : "opacity-50"}`}
          />
        </button>
      </div>
      ...
      <div className="absolute inset-0 flex items-center justify-center pointer-events-none">
        <div
          className={`glass p-4 sm:p-6 max-w-xs sm:max-w-sm text-center transition-opacity duration-1000 mx-4 ${
            isVisible ? "opacity-70" : "opacity-0"
          }`}
        >
          <div className="text-white/80 text-sm space-y-2">
            <div className="text-xl sm:text-2xl mb-3">👋</div>
            <div className="text-xs sm:text-sm">Arrastra para rotar el objeto</div>
            {experience.assets.length > 1 && (
              <div className="text-xs sm:text-sm">Usa los controles para cambiar contenido</div>
            )}
