  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-950 via-brand-950 to-qr-950 flex items-center justify-center relative overflow-hidden">
      ...
      <div className="text-center text-white max-w-md mx-4 z-10">
        {/* Error icon */}
        <div className="text-7xl mb-6 animate-pulse">{getErrorIcon()}</div>

        {/* Error title */}
        <div className="text-2xl font-semibold mb-4">{getErrorTitle()}</div>
        ...
        <div className="mb-8">
          ...
          <div className="space-y-2">
            {getErrorSuggestions().map((suggestion, index) => (
              <div
                key={index}
                className="text-xs text-blue-200/70 glass p-2 rounded backdrop-blur-sm"
              >
                • {suggestion}
              </div>
            ))}
        ...
        {/* Quick AR branding */}
        <div className="mt-6 text-xs text-white/30">Quick AR Experience</div>
      </div>
