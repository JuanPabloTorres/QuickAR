...
export default function Interactive3DViewer({
  ...
  useEffect(() => {
    ...
    import("three").then(async (THREE) => {
      ...
      // Ground - very subtle or invisible for AR effect
      const groundGeometry = new THREE.PlaneGeometry(100, 100);
      const groundMaterial = new THREE.ShadowMaterial({
        opacity: 0.05, // Almost invisible for AR
      });
      ...
      // Optional: Very subtle grid for spatial reference (can be removed)
      const gridHelper = new THREE.GridHelper(50, 50, 0xd0d0d0, 0xe8e8e8);
      gridHelper.material.opacity = 0.03; // Almost invisible for AR
      gridHelper.material.transparent = true;
      gridHelper.position.y = 0.001;
      scene.add(gridHelper);

      // NO PLATFORM - Assets float in AR space
