...
const useProgress = dynamic(
  () => import("@react-three/drei").then((mod) => mod.useProgress),
  { ssr: false }
) as any;

// Import UniversalAssetRenderer (assuming it doesn't use Three.js directly)
      ...
      {experience.assets[selectedAssetIndex] && (
        <spotLight
          position={[
            ...
            ).map((v, i) => v + (i === 1 ? 5 : 0)) as [number, number, number]),
          ]}
          target-position={getAssetPosition(
            selectedAssetIndex,
            experience.assets.length
          )}
          angle={0.3}
...
export default function Desktop3DViewer({
  ...
  // Show loading spinner while mounting on client
  if (!isMounted) {
    return (
          ...
          <div className="flex items-center justify-between pointer-events-auto">
            ...
            <div className="text-right text-white">
              ...
              <p className="text-sm text-blue-200">
                Vista 3D Interactiva • {selectedIndex + 1} /{" "}
                {experience.assets.length}
              </p>
            </div>
          ...
          <div className="absolute left-4 top-1/2 -translate-y-1/2 pointer-events-auto">
            <div className="bg-black/70 backdrop-blur text-white text-xs px-4 py-3 rounded-lg max-w-xs relative">
              ...
              <ul className="space-y-1">
                <li>
                  🖱️ <strong>Click:</strong> Seleccionar objeto
                </li>
                <li>
                  🖱️ <strong>Arrastrar:</strong> Rotar vista
                </li>
                <li>
                  🖱️ <strong>Rueda:</strong> Zoom
                </li>
                <li>
                  🖱️ <strong>Click derecho:</strong> Desplazar
                </li>
                <li>
                  👆 <strong>Toca objetos</strong> para interactuar
                </li>
              </ul>
            </div>
          </div>
