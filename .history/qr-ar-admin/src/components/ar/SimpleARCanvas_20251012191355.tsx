 ...
 */      {/* AR Info Overlay */}
        ...
        <div className="text-sm">
          ...
          <div className="text-xs opacity-75">
            {firstModel.assetUrl?.startsWith('blob:') ? (
              <span className="text-red-300">⚠️ Blob URL (fixed)</span>
            ) : (
      ...
      <div className="absolute bottom-20 left-4 right-4">
        ...
        >
          ...
          <div>URL: {modelUrl}</div>
          <div className="text-xs mt-1 opacity-75">
            {firstModel.assetUrl?.startsWith('blob:') 
              ? 'Blob URLs are temporary and cannot be used for model loading' 
...
function ModelViewerARScene({ experience }: { experience: Experience }) {
  ...
  // Fix blob URLs - convert to proper server URLs or fallback
  let modelUrl = firstModel.assetUrl || "/models/default-model.glb";
  
  if (modelUrl.startsWith('blob:')) {
    console.warn('⚠️ Blob URL detected, this will not work for model loading:', modelUrl);
    // Try to get filename from asset name or ID and construct proper path
    const filename = firstModel.name.toLowerCase().replace(/\s+/g, '-') + '.glb';
    modelUrl = `/uploads/models/${filename}`;
    console.log('🔧 Attempting fallback URL:', modelUrl);
  }
