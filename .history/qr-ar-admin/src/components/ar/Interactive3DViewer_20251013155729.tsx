...
import { ArrowLeft, ChevronLeft, ChevronRight, Info, MousePointer2 } from "lucide-react";
...
export default function Interactive3DViewer({
  ...
  useEffect(() => {
    ...
    import("three").then(async (THREE) => {
      ...
      // Scene setup - realistic environment
      const scene = new THREE.Scene();
      scene.background = new THREE.Color(0x87ceeb); // Sky blue
      scene.fog = new THREE.Fog(0x87ceeb, 15, 50); // Atmospheric fog
      ...
      camera.position.set(0, 1.6, 6); // 1.6m = average human eye level
      ...
      const getAssetPosition = (index: number, total: number): [number, number, number] => {
        ...
        switch (asset.assetType) {
          ...
          case "image": {
            if (asset.assetUrl) {
              ...
              textureLoader.load(imageUrl, (texture) => {
                ...
                canvasGroup.add(topFrame, bottomFrame, leftFrame, rightFrame, imagePlane);
                canvasGroup.castShadow = true;
                [topFrame, bottomFrame, leftFrame, rightFrame].forEach(f => f.castShadow = true);
          ...
          case "video": {
            ...
            const tvBody = new THREE.Mesh(
              new THREE.BoxGeometry(screenWidth + 0.3, screenHeight + 0.3, 0.15),
              new THREE.MeshStandardMaterial({ 
                color: 0x111111,
                roughness: 0.2,
                metalness: 0.8
          ...
          case "webcontent": {
            ...
            const keyboard = new THREE.Mesh(
              new THREE.BoxGeometry(screenWidth + 0.3, screenHeight * 0.6, 0.04),
              new THREE.MeshStandardMaterial({ 
                color: 0x4b5563,
                roughness: 0.5,
                metalness: 0.5
            ...
            [screenBody, screen, keyboard].forEach(p => p.castShadow = true);
    ...
    <div className="relative w-full h-screen bg-gradient-to-b from-sky-400 to-sky-200">
      ...
      {isLoading && (
        <div className="absolute inset-0 flex items-center justify-center bg-slate-900/80 backdrop-blur-sm z-10">
          <div className="text-center">
            ...
            <p className="text-white text-lg font-semibold">Cargando visor 3D interactivo...</p>
          </div>
        </div>
      )}
        ...
        <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 z-20 pointer-events-none">
          <div className="bg-black/70 backdrop-blur-md rounded-2xl p-8 text-white max-w-md animate-fade-in">
            ...
            <div className="space-y-3 text-sm">
              <p>🖱️ <strong>Click izquierdo + arrastrar:</strong> Rotar cámara</p>
              <p>🖱️ <strong>Click derecho + arrastrar:</strong> Mover cámara</p>
              <p>🎯 <strong>Scroll:</strong> Acercar / Alejar</p>
              <p>👆 <strong>Click en objeto:</strong> Seleccionar asset</p>
            </div>
          </div>
        </div>
      )}
      ...
      <div className="absolute bottom-32 left-1/2 -translate-x-1/2 z-20 max-w-lg">
        <div className="bg-white/10 backdrop-blur-md rounded-xl p-4 text-white text-center">
          ...
          {selectedAsset.assetContent && selectedAsset.assetType === "message" && (
            <p className="text-white/80 text-sm mt-2">{selectedAsset.assetContent}</p>
          )}
