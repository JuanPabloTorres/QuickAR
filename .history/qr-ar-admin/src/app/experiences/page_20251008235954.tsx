...
import { ARExperience, ExperienceFilters, LoadingState } from "@/lib/types";
  ...
  useEffect(() => {
    ...
    if (searchTerm.trim()) {
      const filters: ExperienceFilters = {
        search: searchTerm,
        isActive: filterBy === "active" ? true : undefined,
        hasAR: filterBy === "withAR" ? true : undefined,
      };
      filtered = filterExperiences(filtered, filters);
    }
    ...
    // Ordenar
    filtered = sortExperiences(filtered, sortBy);
    ...
    <div className="container mx-auto px-4 py-8">
      ...
      <div className="bg-white rounded-lg shadow-sm border p-6 mb-8">
        ...
        <div className="mt-4 pt-4 border-t flex items-center justify-between text-sm text-gray-500">
          <span>
            Mostrando {filteredExperiences.length} de {experiences.length}{" "}
            experiencias
          </span>
          <div className="flex space-x-4">
            ...
            <span>Con AR: {experiences.filter((exp) => exp.hasAR).length}</span>
          </div>
        </div>
      </div>
      ...
      {filteredExperiences.length === 0 ? (
        <div className="text-center py-12">
          ...
          <Link href="/experiences/create">
            <Button>Crear nueva experiencia</Button>
          </Link>
        </div>
      ) : (
    ...
    <Card className="group hover:shadow-lg transition-all duration-200 border-2 hover:border-blue-200">
      ...
      <div className="p-6">
        ...
        <div className="flex items-center justify-between text-xs text-gray-500 mb-4">
          <span>{formatDate(experience.updatedAt)}</span>
          <span>ID: {experience.id.slice(-6)}</span>
        </div>
