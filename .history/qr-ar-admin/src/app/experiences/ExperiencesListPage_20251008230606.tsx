...
import { ExperienceGrid } from "@/components/ExperienceCube";
...
function FilterBar({
  ...
  const filters = [
    ...
    { key: "ar", label: "Con AR", count: experiencesCount.withAR },
    {
      key: "inactive",
      label: "Inactivas",
      count: experiencesCount.total - experiencesCount.active,
    },
  ];
...
export function ExperiencesListPage({
  ...
  useEffect(() => {
    if (searchParams.sort) {
      const [field, order] = searchParams.sort.split("-") as [
        SortField,
        SortOrder
      ];
      setSortField(field);
      ...
      <main className="container mx-auto px-4 py-8">
        ...
        <div className="mb-6">
          <div className="text-sm text-gray-600">
            Mostrando {paginatedResults.items.length} de{" "}
            {paginatedResults.pagination.total} experiencias
          </div>
        </div>
        ...
        {paginatedResults.pagination.totalPages > 1 && (
          <div className="mt-8 flex justify-center">
            <nav className="flex items-center space-x-2">
              ...
              {/* Números de página */}
              {Array.from(
                { length: paginatedResults.pagination.totalPages },
                (_, i) => i + 1
              )
                .filter(
                  (page) =>
