...
export function ExperienceCreateForm() {
  ...
  const validateForm = useCallback((): ValidationErrors => {
    ...
    if (formData.assets.length === 0) {
      newErrors.assets =
        "Debe agregar al menos un asset para la experiencia AR";
    } else {
      const invalidAssets = formData.assets.filter(
        (asset) => !asset.name || !asset.type || !asset.url
      );

      if (invalidAssets.length > 0) {
        newErrors.assets =
          "Todos los assets deben tener nombre, tipo y archivo";
      }
    }
  ...
  /**
   * Agrega un nuevo asset al formulario
   */
  ...
  const updateAsset = useCallback(
    (index: number, updates: Partial<ARAsset>) => {
      setFormData((prev) => ({
        ...prev,
    ...
    async (e: React.FormEvent) => {
      ...
      try {
        ...
        setErrors((prev) => ({
          ...prev,
          general:
            "Error al crear la experiencia. Verifica los datos e inténtalo de nuevo.",
        }));
      } finally {
        setIsLoading(false);
      }
