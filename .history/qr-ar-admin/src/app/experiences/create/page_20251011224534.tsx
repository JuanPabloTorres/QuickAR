...
import { Asset } from "@/types";  CardDescription,  CardDescription,
...
interface FormData {
  ...
  assetType: AssetType;import { createExperience } from "@/lib/api/experiences";import { createExperience } from "@/lib/api/experiences";

  assetUrl?: string;

  assetContent?: string;import { Asset } from "@/types";import { Asset } from "@/types";
...
export default function CreateExperiencePage() {import { useRouter } from "next/navigation";import { useRouter } from "next/navigation";

  const router = useRouter();

  const [formData, setFormData] = useState<FormData>({import { useState } from "react";import { useState } from "react";
  ...
  const handleInputChange = (interface FormData {interface FormData {
    ...
    setFormData((prev) => ({ ...prev, [name]: value }));  assetType: AssetType;  assetType: AssetType;
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setError(null);  file?: File;  file?: File;
    ...
    try {}}
      ...
      if (formData.assetType !== "message" && !formData.assetUrl) {
        ...
        return;export default function CreateExperiencePage() {export default function CreateExperiencePage() {
  ...
  const router = useRouter();  const router = useRouter();
      ...
      // Create the asset based on type    description: "",    description: "",

      const asset = {

        name: formData.title || "Asset sin nombre", // Use title as asset name, with fallback    assetType: "image",    assetType: "image",

        kind: formData.assetType,

        url: formData.assetType !== "message" ? (formData.assetUrl || undefined) : undefined,  });  });

        text: formData.assetType === "message" ? formData.assetContent : undefined,

        mimeType: formData.file?.type,  const [isLoading, setIsLoading] = useState(false);  const [isLoading, setIsLoading] = useState(false);
      ...
      const experienceData = {
        ...
        title: formData.title,  const handleInputChange = (  const handleInputChange = (

        description: formData.description,

        assets: [asset], // Backend expects an array of assets    e: React.ChangeEvent<    e: React.ChangeEvent<
    ...
    >    >
      ...
      if (response.success) {
        ...
        router.push("/experiences");    const { name, value } = e.target;    const { name, value } = e.target;

      } else {

        setError(response.message || "Error creando la experiencia");    setFormData((prev) => ({ ...prev, [name]: value }));    setFormData((prev) => ({ ...prev, [name]: value }));
  ...
  return (
    ...
    <div className="container mx-auto px-4 py-8 max-w-4xl">    setIsLoading(true);    setIsLoading(true);

      {/* Header */}

      <div className="mb-8">    setError(null);    setError(null);
        ...
        <h1 className="text-3xl font-bold text-white mb-2">      if (formData.assetType !== "message" && !formData.assetUrl) {      if (formData.assetType !== "message" && !formData.assetUrl) {

          Crear Nueva Experiencia

        </h1>        setError("Por favor, sube un archivo o proporciona una URL");        setError("Por favor, sube un archivo o proporciona una URL");

        <p className="text-blue-200">

          Configura tu experiencia de realidad aumentada        return;        return;
      ...
      {error && (
        ...
        <div className="mb-6 p-4 bg-red-500/10 border border-red-500/20 rounded-lg">
          ...
          <p className="text-red-400">{error}</p>      if (formData.assetType === "message" && !formData.assetContent) {      // Prepare data based on asset type
      ...
      <form onSubmit={handleSubmit} className="space-y-8">        return;        title: formData.title,
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
          ...
          <Card className="glass">
            ...
            <CardHeader>        assetType: formData.assetType,

              <CardTitle className="text-xl text-white">

                Información Básica      // Create the asset based on type        assetUrl: formData.assetUrl || "",

              </CardTitle>

              <CardDescription>      const asset = {        assetContent: formData.assetContent || "",

                Detalles generales de la experiencia

              </CardDescription>        name: formData.title || "Asset sin nombre", // Use title as asset name, with fallback      };
