...
export default function ARExperience() {
  ...
  const checkARCapabilities = useCallback(async () => {
    try {
      ...
      setArState(prev => ({ ...prev, hasCamera: false, isWebXRSupported: false }));
    }
  }, []);
  ...
  const renderARObject = useCallback((ctx: CanvasRenderingContext2D, obj: ARObject, canvas: HTMLCanvasElement) => {
  ...
  const renderWebAsset = (ctx: CanvasRenderingContext2D, asset: Asset, x: number, y: number, size: number) => {
    const radius = size / 2;
  ...
  const handleCanvasTouch = useCallback((event: React.TouchEvent | React.MouseEvent) => {
    ...
    if (touchedObject) {
      setSelectedObject(prev => prev === touchedObject.id ? null : touchedObject.id);
    } else {
        ...
        <div className="absolute bottom-0 left-0 right-0 z-20 bg-gradient-to-t from-black/80 to-transparent p-4">
          <div className="text-white">
            ...
            <div className="flex items-center justify-between text-sm">
              <span>{arObjects.filter(obj => obj.visible).length} objetos visibles</span>
              {selectedObject && (
                <span className="text-green-400">
                  ✨ {arObjects.find(obj => obj.id === selectedObject)?.asset.name}
                </span>
              )}
            </div>
          </div>
        ...
        <div className="text-center text-white max-w-lg">
          ...
          <div className="bg-white/10 backdrop-blur rounded-2xl p-6 mb-8">
            ...
            <p className="mb-4 text-lg">
              {arObjects.length} objeto{arObjects.length !== 1 ? 's' : ''} digital{arObjects.length !== 1 ? 'es' : ''} en realidad aumentada
            </p>
            ...
            <div className="grid grid-cols-2 gap-3 mb-6">
              {arObjects.slice(0, 4).map((obj, index) => (
                <div key={index} className="bg-white/10 rounded-lg p-3">
                  ...
                  <div className="text-xs text-blue-200 capitalize">{obj.asset.assetType}</div>
                </div>
              ))}
              {arObjects.length > 4 && (
          ...
          <div className="mb-8 space-y-3">
            <div className={`flex items-center justify-center gap-2 ${arState.hasCamera ? 'text-green-400' : 'text-red-400'}`}>
              {arState.hasCamera ? '✅' : '❌'} Cámara {arState.hasCamera ? 'disponible' : 'no disponible'}
            </div>
            
            <div className={`flex items-center justify-center gap-2 ${window.isSecureContext ? 'text-green-400' : 'text-yellow-400'}`}>
              {window.isSecureContext ? '🔒' : '⚠️'} {window.isSecureContext ? 'Conexión segura' : 'Requiere HTTPS'}
            </div>
          ...
          >
            {!window.isSecureContext ? '❌ Requiere conexión HTTPS' :
