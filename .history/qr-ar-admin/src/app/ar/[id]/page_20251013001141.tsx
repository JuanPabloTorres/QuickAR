...
import { getExperienceById, getE        console.log("✅ Camera opened successfully!");
        console.log("🎨 Starting AR rendering...");
        startARRendering();
        
        // Forzar un renderizado inmediato para test
        setTimeout(() => {
          if (canvasRef.current) {
            const canvas = canvasRef.current;
            const ctx = canvas.getContext("2d");
            if (ctx && experience?.assets[currentAssetIndex]) {
              console.log("🔥 FORCED RENDER TEST");
        ...
        setIsStartingAR(false);ienceBySlug } from "@/lib/api/experiences";
...
export default function ARExperience() {
  ...
  const handleTouchMove = (e: React.TouchEvent) => {
    ...
    if (isDragging && e.touches.length === 1) {
      ...
      setArObjectScale(prev => Math.max(0.3, Math.min(3.0, prev * scaleFactor)));
  ...
  const startARRendering = () => {
    ...
    const renderFrame = () => {
      if (video.readyState >= 2 && isARActive) {
        ...
        console.log("🎨 Drawing AR content - Canvas size:", canvas.width, "x", canvas.height);
        console.log("🎯 AR Object position:", arObjectPosition, "scale:", arObjectScale);
  ...
  const drawARContent = (
    ...
    if (!currentAsset) {
      console.warn("❌ No current asset to draw");
      return;
    }

    console.log("🎨 Drawing AR content for asset:", currentAsset.name, currentAsset.assetType);

    // Calculate position based on relative coordinates
    const arX = width * arObjectPosition.x;
    const arY = height * arObjectPosition.y;
    
    console.log("📍 AR position:", { arX, arY, width, height, position: arObjectPosition, scale: arObjectScale });

    // CÍRCULO DE PRUEBA MUY VISIBLE
    ctx.save();
    ctx.fillStyle = "rgba(255, 0, 0, 0.8)"; // Rojo semi-transparente
    ctx.beginPath();
  ...
  const drawTextAR = (
    ...
    const textWidth = ctx.measureText(asset.assetContent).width + (40 * arObjectScale);
          ...
          <div className="absolute bottom-4 left-4 right-4 pointer-events-auto z-50">
            ...
            <div className="flex justify-center mb-4">
              <div className="flex space-x-2 bg-black/70 backdrop-blur-sm px-4 py-2 rounded-lg">
                ...
                <button
                  onClick={() => setArObjectScale(prev => Math.min(3.0, prev * 1.2))}
                  className="bg-green-600 hover:bg-green-700 px-3 py-1 rounded text-white text-sm"
                >
                  🔍 +
                </button>
                <button
                  onClick={() => setArObjectScale(prev => Math.max(0.3, prev * 0.8))}
