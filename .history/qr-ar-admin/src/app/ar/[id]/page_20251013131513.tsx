...
export default function ARExperience() {
  ...
  // Start AR rendering loop (without camera)
  const startARRendering = () => {
    console.log("🎥 Starting AR rendering loop (no camera mode)");
    const canvas = document.getElementById('arCanvas') as HTMLCanvasElement;
    if (!canvas) {
      console.error("❌ Canvas not found for AR rendering");
      return;
    }
    ...
    const renderFrame = () => {
      ...
      // Clear canvas with a dark purple background (simulate no camera)
      ctx.fillStyle = '#2d1b69'; // Dark purple background
      ctx.fillRect(0, 0, canvas.width, canvas.height);
      
      // Add visual indicators that AR is working
      ctx.fillStyle = 'white';
      ctx.font = 'bold 24px Arial';
      ctx.textAlign = 'center';
      ctx.fillText(`🔮 AR Test Mode - Frame ${frameCount}`, canvas.width / 2, 50);
      ctx.fillText('Purple Background = No Camera Required', canvas.width / 2, 80);
  ...
  const startAR = async () => {
    ...
    try {
      ...
      // Try to get camera, but continue even if it fails
      if (!window.isSecureContext) {
        console.warn("⚠️ No secure context, will show AR without camera");
        cameraWorking = false;
      }
  ...
  const startARRendering = () => {
    ...
    const renderFrame = () => {
      if (video.readyState >= 2 && isARActive) {
        ...
        // FORCE IMMEDIATE VISIBLE CONTENT
        ctx.fillStyle = "rgba(255, 0, 0, 0.5)"; // Semi-transparent red background
        ctx.fillRect(0, 0, canvas.width, canvas.height);

        // FORCE BIG TEXT IN CENTER
        ctx.fillStyle = "white";
        ctx.font = "bold 40px Arial";
        ctx.textAlign = "center";
        ctx.textBaseline = "middle";
        ctx.fillText("AR WORKING!", canvas.width / 2, canvas.height / 2);

        // DEBUG: Draw corner markers to verify canvas positioning
        ctx.fillStyle = "yellow";
        ctx.fillRect(10, 10, 50, 50); // Top-left corner
        ctx.fillRect(canvas.width - 60, 10, 50, 50); // Top-right corner
        ctx.fillRect(10, canvas.height - 60, 50, 50); // Bottom-left corner
        ctx.fillRect(canvas.width - 60, canvas.height - 60, 50, 50); // Bottom-right corner

        // Dibujar el contenido AR real
        drawARContent(ctx, canvas.width, canvas.height);
      }
  ...
  const drawARContent = (
    ...
    // FORCE DRAW BIG VISIBLE CONTENT - NO MATTER WHAT
    ctx.save();

    // Draw MASSIVE red circle in center
    ctx.fillStyle = "rgba(255, 0, 0, 0.8)";
    ctx.beginPath();
    ...
    ctx.textAlign = "center";
    ctx.textBaseline = "middle";
    ctx.fillText("AR CONTENT", width / 2, height / 2);

    // Draw asset info
    ctx.font = "bold 20px Arial";
    ctx.fillText(currentAsset.name, width / 2, height / 2 + 40);
    ctx.fillText(`Type: ${currentAsset.assetType}`, width / 2, height / 2 + 70);

    ctx.restore();

    // Then draw the actual content based on type
