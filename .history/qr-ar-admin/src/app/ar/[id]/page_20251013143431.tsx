...
export default function ARExperience() {
  ...
  const renderMessageAsset = useCallback((ctx: CanvasRenderingContext2D, asset: Asset) => {
    ...
    const gradient = ctx.createLinearGradient(-maxWidth/2, -50, maxWidth/2, 50);
  ...
  const renderImageAsset = useCallback((ctx: CanvasRenderingContext2D, asset: Asset) => {
  ...
  const renderVideoAsset = useCallback((ctx: CanvasRenderingContext2D, asset: Asset) => {
    const size = 150;
  ...
  const render3DAsset = useCallback((ctx: CanvasRenderingContext2D, asset: Asset) => {
    const size = 150;
  ...
  const renderWebContentAsset = useCallback((ctx: CanvasRenderingContext2D, asset: Asset) => {
    const size = 150;
    ...
    ctx.roundRect(-browserWidth/2, -browserHeight/2, browserWidth, browserHeight, 8);
    ctx.fill();
    
    // Barra de navegador
    ctx.fillStyle = 'rgba(107, 114, 128, 0.1)';
    ctx.roundRect(-browserWidth/2, -browserHeight/2, browserWidth, browserHeight*0.25, 8);
    ctx.fill();
    
    // Puntos del navegador
    [-browserWidth*0.35, -browserWidth*0.25, -browserWidth*0.15].forEach((x, index) => {
    ...
    for(let i = 0; i < 3; i++) {
      ...
      ctx.fillRect(-lineWidth/2, -browserHeight*0.1 + i*8, lineWidth, 3);
    }
