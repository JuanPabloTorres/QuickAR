...
export default function ARExperience() {
  ...
  const loadExperience = useCallback(async () => {
    try {
      ...
      // - Solo números: ID numérico -> /api/experiences/{id}
      // - UUID (formato: xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx): -> /api/experiences/{id}
      // - Cualquier otra cosa: Slug -> /api/experiences/slug/{slug}
      const isNumericId = /^[0-9]+$/.test(experienceId);
      const isUUID =
      ...
      if (isNumericId || isUUID) {
        console.log("📍 Usando endpoint por ID:", experienceId);
        data = await getExperienceById(experienceId);
      } else {
        console.log("📍 Usando endpoint por slug:", experienceId);
        data = await getExperienceBySlug(experienceId);
      }
  ...
  // Render the unified AR viewer (automatically chooses mobile AR or desktop 3D)
  return (
    <UnifiedARViewer
