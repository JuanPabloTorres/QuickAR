...
export default function ARExperience() {
  ...
  const getRelativePosition = (clientX: number, clientY: number) => {
    ...
    console.log("📍 Position update:", { clientX, clientY, rect, result: { x, y } });
    return { x, y };
  };
  ...
  const handleTouchStart = (e: React.TouchEvent) => {
    ...
    if (e.touches.length === 1) {
      ...
      setArInteraction((prev) => ({
        ...
        isDragging: true,
        isScaling: false,
        position: pos, // Actualizar posición inmediatamente
      }));
      ...
      console.log("🔍 Started scaling with distance:", distance);
    }
  };
  ...
  const handleMouseMove = (e: React.MouseEvent) => {
    if (arInteraction.isDragging) {
      ...
      const pos = getRelativePosition(e.clientX, e.clientY);
      setArInteraction((prev) => ({ ...prev, position: pos }));
      console.log("🖱️ Mouse dragging to:", pos);
    }
  };
  ...
  const drawMessageAsset = (
    ...
    ctx.fillRect(x - maxWidth / 2 + 5, y - minHeight / 2 + 5, maxWidth, minHeight);
  ...
  const drawARUI = (ctx: CanvasRenderingContext2D, width: number, height: number) => {
  ...
  if (arState.isActive) {
    return (
      <div className="relative w-full h-screen bg-black overflow-hidden">
        ...
        <canvas
          ref={canvasRef}
          className="absolute inset-0 w-full h-full pointer-events-auto z-20 bg-transparent select-none"
          ...
          <div className="absolute bottom-4 left-4 pointer-events-none z-50">
            <div className="bg-black/80 backdrop-blur-sm p-3 rounded-lg border border-cyan-500/30">
              <div className="text-white text-xs font-mono">
                <div className="text-cyan-300 font-bold mb-1">🔧 DEBUG INFO</div>
                <div>Pos: {arInteraction.position.x.toFixed(3)}, {arInteraction.position.y.toFixed(3)}</div>
                <div>Scale: {arInteraction.scale.toFixed(2)}</div>
                <div className={arInteraction.isDragging ? "text-green-400" : "text-gray-400"}>
                  Dragging: {arInteraction.isDragging ? "YES" : "NO"}
                </div>
                <div className={arInteraction.isScaling ? "text-blue-400" : "text-gray-400"}>
                  Scaling: {arInteraction.isScaling ? "YES" : "NO"}
                </div>
                <div>Asset: {currentAssetIndex + 1}/{experience.assets.length}</div>
              </div>
            </div>
          </div>
          ...
          <div className="absolute bottom-4 left-4 right-4 pointer-events-auto z-50">
            ...
            <div className="flex justify-center mb-4">
              <div className="flex space-x-2 bg-black/70 backdrop-blur-sm px-4 py-2 rounded-lg">
                ...
                <button
                  onClick={() => {
                    console.log("🧪 Testing movement - moving to random position");
                    setArInteraction((prev) => ({ 
                      ...prev, 
                      position: { 
                        x: Math.random(), 
