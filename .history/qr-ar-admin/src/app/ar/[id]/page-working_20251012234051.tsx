...
export default function FunctionalARExperience() {
    ...
    const loadExperience = async () => {
      ...
      try {
        ...
        try {
          ...
          setError("Error de conexión. Verifica que el servidor API esté ejecutándose en http://localhost:5001");
          setIsLoading(false);
          return;
        }
  ...
  const startAR = async () => {
    try {
      ...
      setError("No se pudo acceder a la cámara. Verifica los permisos y usa HTTPS.");
    }
  };
  ...
  const drawARContent = (ctx: CanvasRenderingContext2D, width: number, height: number) => {
    ...
    // Draw asset content based on type
  ...
  const draw3DAR = (ctx: CanvasRenderingContext2D, asset: Asset, x: number, y: number) => {
    ctx.save();
          ...
          <div className="absolute top-20 left-4 pointer-events-auto z-50">
            <div className="bg-black/70 backdrop-blur-sm p-3 rounded-lg">
              <div className="text-white">
                <div className="font-medium text-lg">{experience.assets[currentAssetIndex].name}</div>
          ...
          <div className="absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 pointer-events-none z-40">
            <div className="text-center text-white">
              <div className="bg-black/70 backdrop-blur-sm px-6 py-3 rounded-lg mb-4">
                <div className="text-lg font-semibold">🎯 Apunta hacia una superficie</div>
                <div className="text-sm text-blue-200">El contenido AR aparece en el centro</div>
              </div>
            </div>
          </div>
