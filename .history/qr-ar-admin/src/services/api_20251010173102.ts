...
class ApiService {
  ...
  async createExperience(experience: Partial<Experience>): Promise<ApiResponse<Experience>> {
    return this.request<Experience>('/experiences', {
      method: 'POST',
  ...
  async updateExperience(id: string, experience: Partial<Experience>): Promise<ApiResponse<Experience>> {
    return this.request<Experience>(`/experiences/${id}`, {
      method: 'PUT',
      body: JSON.stringify(experience),
    });
  ...
  // File Upload API
  async uploadFile(file: File, type: 'model' | 'image' | 'video'): Promise<ApiResponse<{ url: string }>> {
