...
export async function resolveAssetUrl(
  ...
  console.log("🔍 Resolving asset URL:", { assetUrl, assetName, assetId, assetType });

  // If no URL provided, try to construct one
  if (!assetUrl) {
    const constructedUrl = constructAssetUrl(assetType, assetName, assetId, baseApiUrl);
  ...
  if (assetUrl.startsWith("blob:")) {
    ...
    if (isValid) {
      ...
      const serverUrl = constructAssetUrl(assetType, assetName, assetId, baseApiUrl);
      return await validateAndReturnUrl(serverUrl, false);
    }
  }

  // For web content, return as-is if it looks like a URL
  if (assetType === "webcontent" && (assetUrl.includes("http") || assetUrl.includes("www."))) {
    return { url: assetUrl, isBlob: false, isValid: true };
  }

  // Default: try to construct a server URL
  const fallbackUrl = constructAssetUrl(assetType, assetName, assetId, baseApiUrl);
...
async function validateAndReturnUrl(url: string, isBlob: boolean): Promise<AssetUrlResult> {
  try {
 ...
 */
export async function createBlobUrlFromServer(serverUrl: string): Promise<string | null> {
  try {
