...
export class QuickARApiClient {
  ...
  async getExperiences(): Promise<ARExperience[]> {
    const response = await this.makeRequest<ApiResponse<ExperienceDto[]>>(
      "/api/experiences"
    );

    if (!response.success || !response.data) {
      throw new APIError(
        response.message || "Failed to fetch experiences",
        500
      );
    }
  ...
  async createExperience(experience: {
    ...
    if (!response.success || !response.data) {
      throw new APIError(
        response.message || "Failed to create experience",
        500
      );
    }
  ...
  async uploadAsset(file: File): Promise<FileUploadResult> {
    ...
    const response = await this.makeRequest<ApiResponse<FileUploadResult>>(
      "/api/upload",
      {
        method: "POST",
        body: formData,
  ...
  async trackEvent(event: {
    ...
    const response = await this.makeRequest<ApiResponse<AnalyticsEventDto>>(
  ...
  async getAnalytics(experienceId: string): Promise<AnalyticsEvent[]> {
    const response = await this.makeRequest<ApiResponse<AnalyticsEventDto[]>>(
      `/api/analytics/experiences/${experienceId}`
    );
