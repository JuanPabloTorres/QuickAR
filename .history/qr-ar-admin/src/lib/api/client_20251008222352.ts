...
class ApiClient {
  ...
  private mapExperienceFromDto(dto: ExperienceDto): ARExperience {
    return {
      ...
      primaryAsset: dto.assets?.[0]
        ? this.mapAssetFromDto(dto.assets[0])
        : undefined,
    };
  }

  // Mapper: ExperienceCreateDto
  private mapExperienceToCreateDto(data: {
    title: string;
    slug?: string;
    description?: string;
  }): ExperienceCreateDto {
  ...
  private mapExperienceToUpdateDto(data: {
    title: string;
    slug?: string;
    description?: string;
    isActive?: boolean;
  ...
  async getExperiences(): Promise<ARExperience[]> {
    const response = await this.request<ApiResponse<ExperienceDto[]>>(
      "/api/experiences"
    );
    ...
    return response.data.map((dto: ExperienceDto) =>
      this.mapExperienceFromDto(dto)
    );
  }
  ...
  async getAnalytics(experienceId: string): Promise<AnalyticsEventDto[]> {
    const response = await this.request<ApiResponse<AnalyticsEventDto[]>>(
      `/api/analytics/${experienceId}`
    );
