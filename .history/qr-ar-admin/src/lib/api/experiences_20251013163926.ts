...
const getApiBaseUrl = () => {
  ...
  // Client-side: Try to determine if accessing from network or localhost
  if (typeof window !== "undefined") {
    const hostname = window.location.hostname;
    
    // If hostname is an IP address (network access), use it for API
    if (/^(\d{1,3}\.){3}\d{1,3}$/.test(hostname)) {
      // Network IP detected - use same IP for API on port 5001
      return `http://${hostname}:5001/api`;
    }
    
    // If hostname is "0.0.0.0", we're in network mode but need to use actual IP
    // This is a fallback, ideally NEXT_PUBLIC_API_BASE_URL should be set
    if (hostname === "0.0.0.0") {
      console.warn("⚠️ Detected 0.0.0.0 - using localhost. For network access, set NEXT_PUBLIC_API_BASE_URL");
    }
  }
