import { AssetDto, ExperienceDto, Asset, Experience } from '@/types';
 ...
 * Validate asset type
 */
export function isValidAssetType(type: string): type is 'message' | 'video' | 'image' | 'model3d' {
...
export function getSupportedMimeTypes(assetType: string): string[] {
  const mimeTypes: Record<string, string[]> = {
    ...
    'model3d': ['model/gltf+json', 'model/gltf-binary', 'application/octet-stream'],
    'message': ['text/plain']
  };
 ...
 */
export function validateAssetFile(file: File, assetType: string): { valid: boolean; error?: string } {
  if (assetType === 'message') {
    return { valid: false, error: 'Los assets de mensaje no requieren archivo' };
  }
