 ...
 */
export function mapExperienceDtoToARExperience(
...
export function mapARAssetToAssetDto(asset: ARAsset): Omit<AssetDto, "id"> {
  // Map frontend assetType back to backend kind
  const reverseMapping: Record<ARAsset["assetType"], AssetKind> = {
    text: "message",
    image: "image",
    video: "video",
    model3d: "model3d",
  ...
  const kind = reverseMapping[asset.assetType];
  if (!kind) {
...
export function validateExperience(
  ...
  if (!experience.assets || experience.assets.length === 0) {
    ...
    // Validate each asset
    experience.assets.forEach((asset: ARAsset, index: number) => {
      if (!asset.name?.trim()) {
      ...
      switch (asset.assetType) {
        case "text":
          if (!asset.assetContent?.trim()) {
            errors.push(
              `Asset ${index + 1}: El contenido de texto es requerido`
            );
          }
          break;
        case "image":
...
export function validateAsset(
  ...
  if (asset.fileSizeBytes && asset.fileSizeBytes > maxFileSize) {
    errors.push(
      `El archivo excede el tamaño máximo permitido (${Math.round(
  ...
  if (requireARSettings && asset.assetType === "model3d") {
    if (
      !asset.assetUrl?.endsWith(".glb") &&
      !asset.assetUrl?.endsWith(".gltf")
    ) {
      warnings.push(
        "Se recomienda usar formatos .glb o .gltf para modelos 3D en AR"
      );
    }
  }
...
export function filterExperiences(
  ...
  return experiences.filter((experience) => {
    ...
    if (filters.assetKinds && filters.assetKinds.length > 0) {
      const hasMatchingAsset = experience.assets.some((asset: ARAsset) => {
        const reverseMapping: Record<ARAsset["assetType"], AssetKind> = {
          text: "message",
          image: "image",
          video: "video",
        ...
        return filters.assetKinds!.includes(reverseMapping[asset.assetType]);
      });
    ...
    if (filters.hasAR !== undefined) {
      const hasARAssets = experience.assets.some(
        (asset: ARAsset) => asset.assetType === "model3d"
      );
    ...
    if (filters.searchTerm) {
      ...
      const matchesDescription = experience.description
        ?.toLowerCase()
        .includes(searchTerm);
      const matchesAssetName = experience.assets.some((asset: ARAsset) =>
...
export function getPrimaryAsset(experience: ARExperience): ARAsset | null {
  ...
  const priorityOrder: ARAsset["assetType"][] = [
    "model3d",
    "image",
    "video",
    "text",
  ...
  for (const type of priorityOrder) {
    const asset = experience.assets.find((a: ARAsset) => a.assetType === type);
 ...
 */
export function generateExperienceQRUrl(
...
export function getAssetTypeIcon(assetType: ARAsset["assetType"]): string {
  const iconMap: Record<ARAsset["assetType"], string> = {
    text: "FileText",
    image: "Image",
    video: "Play",
    model3d: "Package",
