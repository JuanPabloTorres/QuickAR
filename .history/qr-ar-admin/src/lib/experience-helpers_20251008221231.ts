...
export function validateAsset(
  ...
  switch (assetType) {
    case 'image':
      ...
      if (!imageFormats.includes(imageExtension) && 
    ...
    case 'model3d':
      ...
      if (modelExtension === 'gltf') {
        warnings.push('Se recomienda usar GLB en lugar de GLTF para mejor rendimiento')
      }
      break
  }

  // Validaciones adicionales
  if (file.size < 100) { // Menos de 100 bytes
    warnings.push('El archivo parece muy pequeño, verifique que no esté corrupto')
  }
...
export function filterExperiences(
  ...
  return experiences.filter(experience => {
    ...
    if (filters.dateRange) {
      ...
      if (experienceDate < filters.dateRange.start || 
 ...
 */
export function getAssetTypeCount(experience: ARExperience): Record<ARAssetType, number> {
  const counts = {
    text: 0,
    image: 0,
    video: 0,
