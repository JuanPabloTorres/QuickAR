...
export class SimpleUploadService {
   ...
   */
  static validateFile(file: File, assetType: string): { isValid: boolean; error?: string } {
    if (!file) {
      return { isValid: false, error: 'No se ha seleccionado ningún archivo' };
    }

    const config = this.FILE_CONFIGS[assetType as keyof typeof this.FILE_CONFIGS];
    if (!config) {
      return { isValid: false, error: `Tipo de archivo no soportado: ${assetType}` };
    }
   ...
   */
  static async uploadFile(file: File, assetType: string): Promise<SimpleUploadResult> {
    try {
      // Validar archivo
