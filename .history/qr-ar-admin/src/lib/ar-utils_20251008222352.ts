...
import { AR3DSettings, ARAsset, ARCapabilities, ARState } from "@/types";
...
export function setupAREventListeners(
  ...
  if (listeners.onError) {
    element.addEventListener("error", (event: any) => {
      const errorMessage =
        event.detail?.error?.message || "Error desconocido en AR";
...
export function validateARAsset(asset: ARAsset): {
  ...
  if (asset.fileSizeBytes) {
    ...
    if (sizeMB > 10) {
      warnings.push(
        `El archivo es grande (${sizeMB.toFixed(
          1
        )}MB). Puede afectar el rendimiento`
      );
    }
  ...
  if (asset.fileSizeBytes) {
    ...
    if (sizeMB > 5) {
      ...
      recommendations.push(
        "Considera comprimir el modelo o reducir la geometría"
      );
    }
  }
