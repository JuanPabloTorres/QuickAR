...
import AdvancedARViewer from "./AdvancedARViewer";
import useARSupport from "./hooks/useARSupport";
import "./styles/AdvancedARViewer.css";
...
export function ExperienceCube({
  ...
  const [useAdvancedAR, setUseAdvancedAR] = useState(enableAdvancedAR);
              ...
              <div className="absolute top-2 right-2 flex gap-1">
                ...
                {enableAdvancedAR && (
                  <motion.button
                    whileHover={{ scale: 1.1 }}
                    whileTap={{ scale: 0.9 }}
                    className="p-2 bg-purple-500/80 text-white rounded-lg backdrop-blur-sm"
                    title="AR Avanzado"
                    onClick={() => setUseAdvancedAR(true)}
                  >
  ...
  // Advanced AR Mode
  if (useAdvancedAR && enableAR && arAvailable) {
    return (
      <motion.div
        className={`experience-cube-ar relative bg-gradient-to-br from-gray-900/5 to-gray-800/10 backdrop-blur-sm rounded-2xl shadow-lg border-2 border-blue-400/50 overflow-visible ${className}`}
        style={{
          width: size,
          height: size,
          minWidth: 300,
      ...
      >
        ...
        <div className="absolute top-3 left-3 z-30">
          <motion.div
            ...
            animate={{ opacity: 1, scale: 1 }}
            className="flex items-center gap-2 px-3 py-1.5 bg-gradient-to-r from-purple-500/90 to-blue-500/90 text-white text-xs font-medium rounded-full backdrop-blur-sm border border-white/30 shadow-lg"
          >
        ...
        <div className="absolute top-3 right-3 z-30">
          <motion.button
            whileHover={{ scale: 1.1 }}
            whileTap={{ scale: 0.9 }}
            onClick={() => setUseAdvancedAR(false)}
            className="p-2 bg-black/60 text-white text-xs rounded backdrop-blur-sm border border-white/20 hover:bg-black/80 transition-colors"
            title="Cambiar a AR básico"
          >
            Básico
          </motion.button>
        </div>
        ...
        <div className="w-full h-full p-4">
          <AdvancedARViewer
            ...
            onInteraction={(event) => {
              console.log("AR interaction:", event);
            }}
          />
        </div>
      </motion.div>
  ...
  return (
    <motion.div
      className={`experience-cube-ar relative bg-gradient-to-br from-gray-900/5 to-gray-800/10 backdrop-blur-sm rounded-2xl shadow-lg border-2 border-gray-300/30 overflow-visible ${
        enableAR && arAvailable ? "ar-ready-pulse" : ""
      } ${className}`}
      style={{
        width: size,
        height: size,
