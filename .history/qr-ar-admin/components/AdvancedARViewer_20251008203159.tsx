...
export function AdvancedARViewer({
  ...
  const initializeARSystems = useCallback(async () => {
    ...
    try {
      ...
      const trackerInitialized = await arTrackerRef.current.initialize(
        canvasRef.current
      );
      ...
      {arState.isInitialized && !isLoading && (
        <>
          ...
          <div className="absolute top-4 left-4 flex flex-col gap-2">
            <div
              className={`px-3 py-1 bg-black/60 text-white text-xs rounded-full backdrop-blur-sm flex items-center gap-2`}
            >
              <div
              ...
              />
              <span
                className={getTrackingQualityColor(arState.trackingQuality)}
              >
