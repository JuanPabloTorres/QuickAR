...
export function ExperienceCubeRefactored({
  ...
  const config = contentTypeConfig[assetKind];
  ...
  useEffect(() => {
    if (assetKind === "model3d" && typeof window !== "undefined") {
  ...
  useEffect(() => {
    if (assetKind === "model3d" && modelViewerRef.current) {
  ...
  }, [assetKind, assetUrl, onARStateChange]);
    ...
    switch (assetKind) {
      ...
      case "model3d":
        ...
        return (
          <div className="relative w-full h-full">
            <div
              ref={modelViewerRef as React.RefObject<HTMLDivElement>}
              className="w-full h-full rounded-lg model-viewer-container model-viewer-background"
              dangerouslySetInnerHTML={{
                __html: `
                  <model-viewer
                    src="${assetUrl}"
                    alt="${alt || "Modelo 3D AR"}"
                    ${!arState.isInAR ? "camera-controls" : ""}
                    ${enableAR && arState.isSupported ? "ar" : ""}
                    ...
                    camera-orbit="${manualControls.rotation.y}deg ${
                  manualControls.rotation.x + 55
                }deg ${4 / manualControls.scale}m"
  ...
  return (
    <div className={`relative ${className}`}>
      <motion.div
        ref={containerRef}
        className="relative bg-white rounded-xl shadow-lg border border-gray-200 overflow-hidden w-full max-w-sm mx-auto sm:max-w-md md:max-w-lg lg:max-w-xl"
        style={{
          width: size,
          height: size,
          minWidth: 280, // Reducido para móviles
          minHeight: 280, // Reducido para móviles
        }}
      ...
      >
        ...
        {/* Manual Controls */}
        {assetKind === "model3d" && !arState.isInAR && (
          <div className="absolute bottom-3 left-3 right-3 z-20">
            <div className="flex flex-wrap sm:flex-nowrap items-center justify-center gap-2 bg-white/90 backdrop-blur-sm rounded-lg p-2 border border-gray-200">
              {/* Movement Controls */}
              <div className="flex items-center gap-1">
                ...
                >
                  <Move className="w-3 h-3 sm:w-4 sm:h-4 rotate-180" />
                </button>
                <button
                  onClick={() => handleMove("x", 0.1)}
                  className="p-1.5 text-gray-600 hover:text-gray-800 hover:bg-gray-100 rounded transition-colors"
        ...
        {assetKind !== "model3d" && enableAR && arState.isSupported && (
          <div className="absolute bottom-3 right-3 z-20">
            <button
              ...
              className="p-2 sm:p-3 bg-blue-600 text-white rounded-full shadow-lg hover:bg-blue-700 transition-colors"
            ...
            <div className="flex items-start justify-between mb-3">
              <h3 className="font-semibold text-gray-900">
                {educationalContent.title ||
                  `Información sobre ${config.label}`}
              </h3>
              <button
