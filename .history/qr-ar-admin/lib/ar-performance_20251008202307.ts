...
export class ARPerformanceOptimizer {
  ...
  private createReducedDetailMesh(original: THREE.Mesh, complexity: number): THREE.Mesh {
  ...
  private getMaterialSignature(material: THREE.Material): string {
    ...
    const props = {
      type: material.type,
      color: material instanceof THREE.MeshStandardMaterial ? material.color.getHex() : null,
      metalness: material instanceof THREE.MeshStandardMaterial ? material.metalness : null,
      roughness: material instanceof THREE.MeshStandardMaterial ? material.roughness : null,
  ...
  private increaseQuality(): void {
    ...
    if (this.settings.textureQuality === 'low') {
      this.settings.textureQuality = 'medium';
      this.optimizeTextures();
    } else if (this.settings.textureQuality === 'medium' && this.deviceInfo.platform === 'desktop') {
      this.settings.textureQuality = 'high';
      this.optimizeTextures();
    }
    
    // Increase shadow map size
    if (this.settings.shadowMapSize < 2048 && this.deviceInfo.platform === 'desktop') {
      this.settings.shadowMapSize *= 2;
      this.updateShadowMaps();
    }
  }
    ...
    this.scene.traverse((object) => {
      if (object instanceof THREE.Mesh) {
        if (object.geometry.boundingSphere) {
          const isVisible = this.frustum.intersectsSphere(object.geometry.boundingSphere);
