...
export class AdvancedARInteractions {
  ...
  private setupTouchGestures(): void {
    ...
    this.canvas.addEventListener("touchmove", (event) => {
      ...
      if (event.touches.length === 1) {
        ...
        if (this.selectedObject) {
          ...
          const scaleChange =
            (currentDistance - this.initialDistance) *
            this.config.pinchSensitivity;
          this.handleObjectScale(this.selectedObject, 1 + scaleChange);

          // Handle rotation
          const rotationChange =
            (currentRotation - this.initialRotation) *
    ...
    this.canvas.addEventListener("touchend", (event) => {
      ...
      const touchDistance = this.lastTouchPosition.distanceTo(
      ...
      if (touchDuration < 200 && touchDistance < this.config.gestureThreshold) {
        ...
        touchDuration > 1000 &&
        touchDistance < this.config.gestureThreshold
      ) {
        // Long press gesture
        this.emitGesture({
          type: "longpress",
        ...
        this.emitGesture({
          ...
          deltaPosition: this.lastTouchPosition
            .clone()
  ...
  private highlightObject(objectId: string): void {
    ...
    const object =
      this.scene.getObjectByName(objectId) ||
  ...
  private playSpatialSound(soundType: string, position: THREE.Vector3): void {
    ...
    const config =
      soundConfigs[soundType as keyof typeof soundConfigs] ||
      soundConfigs.select;
  ...
  private rotateSelectedObject(angle: number = Math.PI / 4): void {
    ...
    const object =
      this.scene.getObjectByName(this.selectedObject) ||
