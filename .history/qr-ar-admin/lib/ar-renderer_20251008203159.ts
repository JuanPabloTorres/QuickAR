...
export class AdvancedARRenderer {
  ...
  constructor(
    ...
    config: Partial<ARRenderingConfig> = {}
  ) {
  ...
  private initializeRenderer(canvas: HTMLCanvasElement): void {
    ...
    // Enable physically based rendering (legacy property, handled by useLegacyLights = false)
  ...
  private updateAnimations(deltaTime: number): void {
    ...
    this.arObjects.forEach((arObject) => {
      ...
      if (arObject.mesh.userData.floatingAnimation) {
        ...
        arObject.mesh.position.y =
          arObject.anchor.y + Math.sin(time * 2) * 0.02;
      }
    });
  }
  ...
  updateLighting(lightEstimation: {
    intensity: number;
    direction: THREE.Vector3;
    color: THREE.Color;
  }): void {
    ...
    this.lightingEnvironment.directionalLight.position.copy(
