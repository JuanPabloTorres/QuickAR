...
export function validateAsset(
  ...
  } else if (!["image", "video", "message", "model3d"].includes(asset.kind)) {
  ...
  if (asset.kind === "message") {
    ...
    } else if (asset.text.length > 2000) {
      warnings.push(
        "Contenido de texto muy largo (máximo recomendado: 2000 caracteres)"
      );
    }
  } else {
    if (!asset.url) {
  ...
  // File size validation (if file size available)
  if (asset.fileSizeBytes && asset.fileSizeBytes > maxFileSize) {
    errors.push(
      `Tamaño de archivo excede el límite (${formatFileSize(maxFileSize)})`
    );
  }

  // MIME type validation
  if (allowedMimeTypes && asset.mimeType) {
  ...
  if (requireARSettings && asset.kind === "model3d") {
    if (!asset.arSettings) {
      warnings.push("Configuración AR recomendada para modelos 3D");
    } else {
      if (
        asset.arSettings.scale &&
        !["auto", "fixed"].includes(asset.arSettings.scale)
      ) {
        const scaleValue = parseFloat(asset.arSettings.scale);
        if (isNaN(scaleValue) || scaleValue <= 0 || scaleValue > 10) {
          warnings.push(
            'Escala AR debe ser "auto", "fixed" o un número entre 0.1 y 10'
          );
        }
      }
    }
...
export function filterExperiences(
  ...
  if (filters.assetKinds && filters.assetKinds.length > 0) {
    filtered = filtered.filter((exp) =>
      exp.assets.some((asset) => filters.assetKinds!.includes(asset.kind))
    );
  }
  ...
  if (filters.hasAR !== undefined) {
    filtered = filtered.filter((exp) => {
      const hasARAssets = exp.assets.some((asset) => asset.kind === "model3d");
      ...
      (exp) =>
        ...
        exp.assets.some(
          (asset) =>
            ...
            asset.mimeType?.toLowerCase().includes(searchTerm)
        )
    );
  }
      ...
      case "assetType":
        ...
        groupKey =
          experience.assets.length > 0
            ? getAssetKindLabel(experience.assets[0].kind)
            : "Sin assets";
        break;
      case "month":
...
export function calculateExperienceStats(experiences: ARExperienceResponse[]): {
  ...
  experiences.forEach((experience) => {
    ...
    // AR availability
    const hasAR = experience.assets.some((asset) => asset.kind === "model3d");
    if (hasAR) {
      stats.withAR++;
    }
    ...
    experience.assets.forEach((asset) => {
      const typeLabel = getAssetKindLabel(asset.kind);
      stats.assetTypes[typeLabel] = (stats.assetTypes[typeLabel] || 0) + 1;
    });
  });

  stats.averageAssetsPerExperience =
    stats.total > 0
...
export function cloneExperience(
  ...
  return {
    ...
    description:
      modifications.description || experience.description || undefined,
    ...
    analyticsEnabled:
      modifications.analyticsEnabled ?? (experience as any).analyticsEnabled,
  };
}
