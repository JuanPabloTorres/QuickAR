import { AssetDto, AssetKind, ExperienceDto } from "@/types";
 ...
 */
export function mapExperienceToDTO(
  experience: ExperienceWithAR
): ExperienceDto {
  return {
    id: experience.id,
  ...
  if (!experience.assets || experience.assets.length === 0) {
    ...
    experience.assets.forEach((asset, index) => {
      ...
      if (asset.kind !== "message" && !asset.url) {
        errors.push(
          `Asset ${index + 1}: URL requerida para este tipo de asset`
        );
      }
...
export function filterExperiences(
  ...
  return experiences.filter((experience) => {
    ...
    if (
      filters.hasAR !== undefined &&
      hasARSupport(experience) !== filters.hasAR
    ) {
      return false;
    }
