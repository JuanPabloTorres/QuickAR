import { Experience, ExperienceDto, AssetDto, AssetKind } from '@/types';
...
// Use unified types from /types/index.ts
export interface ARExperienceResponse extends ExperienceDto {
  qrCodeUrl?: string;
  analyticsEnabled?: boolean;
}

export interface ARAsset extends AssetDto {
  content?: string; // For message/text assets (maps to 'text' field)
  arSettings?: {
    scale?: string;
...
export class ARClientRefactored {
   ...
   */
  async uploadAsset(file: File, onProgress?: (progress: number) => void): Promise<{ url: string; mimeType: string; sizeBytes: number }> {
  ...
  private async detectARSupport(): Promise<{
    ...
    try {
      ...
      support.threejs = (typeof window !== 'undefined' && 'WebGLRenderingContext' in window);
    } catch (error) {
