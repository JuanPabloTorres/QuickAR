...
public class ExperienceService : IExperienceService
{
    ...
    public async Task<ApiResponse<ExperienceDto>> UpdateAsync(Guid id, ExperienceUpdateDto dto, Guid userId)
    {
        try
        {
            _logger.LogInformation("Starting update for experience: {Id} by user: {UserId}", id, userId);
            ...
            {
                _logger.LogWarning("User {UserId} attempted to update experience {Id} owned by {OwnerId}",
                    userId, id, experience.UserId);

                return ApiResponse<ExperienceDto>.ErrorResult("You don't have permission to update this experience");
            }
    ...
    public async Task<ApiResponse<bool>> DeleteAsync(Guid id, Guid userId)
    {
        try
        {
            ...
            if (experience.UserId != userId)
            {
                _logger.LogWarning("User {UserId} attempted to delete experience {Id} owned by {OwnerId}",
                    userId, id, experience.UserId);

                return ApiResponse<bool>.ErrorResult("You don't have permission to delete this experience");
            }
            ...
            _logger.LogInformation("Deleted experience: {Id} by user: {UserId}", id, userId);
    ...
    public async Task<ApiResponse<bool>> ToggleActiveAsync(Guid id, Guid userId)
    {
        try
        {
            var experience = await _context.Experiences.FindAsync(id);
            ...
            if (experience.UserId != userId)
            {
                _logger.LogWarning("User {UserId} attempted to toggle experience {Id} owned by {OwnerId}",
                    userId, id, experience.UserId);

                return ApiResponse<bool>.ErrorResult("You don't have permission to modify this experience");
            }
            ...
            _logger.LogInformation("Toggled active status for experience: {Id} to {IsActive} by user: {UserId}",
                id, experience.IsActive, userId);
