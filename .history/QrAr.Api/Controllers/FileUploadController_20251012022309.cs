...
public class FileUploadEndpointGroup : BaseEndpointGroup
{
    ...
    public override void MapEndpoints(IEndpointRouteBuilder app)
    {
        ...
        // POST /api/v1/upload
        group.MapPost("", UploadFileSimple)
            .WithSummary("Subir archivo (simple)")
            .WithDescription("Sube un archivo detectando automáticamente la categoría")
            .WithEndpointLogging("UploadFileSimple")
            .DisableAntiforgery()
        ...
        // POST /api/upload
        legacyGroup.MapPost("", UploadFileSimple)
            .WithSummary("Subir archivo simple (Legacy)")
            .WithDescription("Sube un archivo detectando automáticamente la categoría")
            .WithEndpointLogging("UploadFileSimple_Legacy")
            .DisableAntiforgery()
            .Accepts<IFormFile>("multipart/form-data")
            .Produces<ApiResponse<FileUploadResult>>(200)
            .Produces<ApiResponse<object>>(400)
    ...
    private static async Task<IResult> UploadFileSimple(IFormFile file, IFileUploadService service)
    {
        // Validar archivo
        if (file == null || file.Length == 0)
        {
        ...
        // Detectar categoría automáticamente basándose en el tipo de archivo
        string category = "images"; // default
        ...
        if (contentType.StartsWith("image/"))
            ...
            category = "videos";
        else if (contentType.Contains("gltf") || contentType.Contains("glb") || file.FileName.EndsWith(".glb") || file.FileName.EndsWith(".gltf"))
            category = "models";
